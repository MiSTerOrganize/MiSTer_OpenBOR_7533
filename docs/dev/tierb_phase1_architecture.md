...
| # | item | where |
|---|---|---|
| 1a | 🛑 **Fetch-engine burst length.** ✅ **CLOSED 2026-08-16 — the choice is FORCED, not preferred.** The row offered two ways to fix a 38-227 MB/s sprite-row-at-a-time fetcher: coalesce to **>= 32-beat (256 B)** reads, **or** keep 2-4 reads outstanding. **The second is not available.** The arbiter's entire correctness argument is that *"both masters are strictly single-outstanding for reads, so exactly one burst is ever in flight and DDRAM_DOUT_READY routes unambiguously to the recorded owner"* (`tierb_ddr_arb.sv:28-29`) — Avalon returns carry **no ID**, so multiple outstanding reads make return-routing ambiguous, which is precisely the `beat_count` corruption 9.8 exists to prevent. **Therefore: coalesce.** Whole-sprite or multi-row prefetch into a staging buffer, >= 32 beats, matching item 0's measured burst-length constraint | item 0, 9.14 M13, `tierb_ddr_arb.sv` |
...
| 6 | **ARM->FPGA register channel** for `compositor_disable`. ✅ **CLOSED 2026-08-16 — it rides the ctrl-word qword the reader ALREADY reads every frame.** 7b established there is no register channel to be had: `h2f_gp` is the only one and it is unusable (`sys/`, and `DO NOT MODIFY sys/` is standing). So §6's *"a control REGISTER, not DDR3"* is not achievable — but it is also not needed. The ctrl qword is read as `ddr_dout[63:0]` and uses only `[31:0]` (ctrl), `[34:32]` (`arm_slot`) and `[47:40]` (`arm_seq`), leaving **21 free bits** — `[39:35]` and `[63:48]`. `compositor_disable` takes one, and **costs zero extra DDR3 transactions**. Precedent is on-core: the pause-menu replay slot already rides this exact spare space. 🛑 The **reader** consumes it and drives a fabric signal to the arbiter — the reader is always alive and is never grant-gated, which is the property 9.11.3 needs and a wedged compositor cannot supply. Latency one frame, which is ample for a disable that precedes a respawn | 9.11.3, 11, §9.11.3.1 |
| 7 | Ring and per-slot **scratch sizing**. ⚠️ **HALF-CLOSED 2026-08-16.** **Scratch: SIZED AND PLACED.** A 960x480 fallback is 921,600 B plus its coverage plane 57,600 B = **979,200 B per slot**, x3 slots = **2,937,600 B (2.80 MB)** — which item 9 makes trivially placeable inside the 383 MB at `0x22040000-0x39FFFFFF`, where before it had no defensible home. **Ring: STILL OPEN**, and it is the harder half — a display list's maximum length falls out of the per-band binning (9.11.4), which is not settled, so the slot size cannot be derived yet. 🛑 Do not close this row on the scratch half alone | 9.11.4, 9.7, item 9 |
...
| # | item | where |
|---|---|---|
| 8 | **The overdraw census RE-RUN.** ✅ **CLOSED 2026-09-14 — THE 450-PAK LIBRARY RUN IS DONE.** 🛑 **The recorded blocker below was the WRONG diagnosis.** `update(1, ...)` was being called and the reports WERE written — `globals.h` makes `printf` into `writeToLogFile`, so every census line went to `OpenBorLog.txt`, never to the stdout/stderr the scan runners capture, and that log is opened `"wt"` at a FIXED path, so parallel jobs truncated each other's copy. Fix: `apply_patches_headless.py` mirrors the engine log to stderr (headless-only; the ship build is untouched), landed on main. **Run:** `ubuntu:24.04`, 6 jobs, 3000 frames, 60 s alarm, binary = CI artifact of `tierb-census-rerun` `47e6aff`, 21 min. **450 PAKs: 442 exit 0, 8 exit 1 (script compile), 426 entered a level, 420 produced reports.** **Fast path 87.8% by COUNT (10,762,961 vs 1,492,450) and 85.4% by TIME (95,042 ms vs 16,213 ms)**; 208 of 420 PAKs take any slow blit, and **64 spend >= 25% of putsprite time on the slow path**. **Slow blend modes:** MULTIPLY **81.5%** (186 PAKs), HALF 7.4%, out-of-range 3.8%, HARDLIGHT 2.5%, OVERLAY 2.2%, opaque 1.7%, SCREEN 0.8%, DODGE 0.1%. What forces them to the CPU is item 14's extension. The census counters remain TEMPORARY DIAG on the branch and must not reach main. *History, superseded:* | 14.5, branch `tierb-census-rerun` |
| 8 (history) | ⚠️ **INSTRUMENT FIXED AND PROVEN ON DEVICE; the LIBRARY-WIDE run is still open.** Branch `tierb-census-rerun`. **T1 fixed** — the slow path is now counted, on a separate `[TBBSLOW]` line so the split stays visible; on He-Man it carried **25% of histogram-counted blits and 5.6% of pixels**, previously recorded as zero. **T5 fixed** — arrays, accumulate cast, printf formats **and the totals** widened to 64-bit (`_tp` sums all eight, so it overflows first and divides every percentage). 🛑 **A THIRD defect found while verifying:** the report gate was **wall-clock only**, which makes a census machine-speed dependent; now `5s OR 600 frames`. **Proven end to end on hardware** — a recorded take drove He-Man into gameplay and the census fired twice with sane numbers. 🛑 **Still blocked for the 450-PAK run:** the gate lives in `update(int ingame, int usevwait)` under `if(ingame == 1 && !_pause)` and **`ingame` is a PARAMETER**; the headless harness reports *"entered level"* from its own marker yet emits nothing, so it is evidently never calling `update(1, ...)`. **One `printf` on `ingame` settles it** — do that first | 14.5, branch `tierb-census-rerun` |
...
| 14 | **The 2.36x headless-vs-device overdraw ratio.** ✅ **CLOSED 2026-08-16 — MEASURED ON DEVICE, and the objection was RIGHT.** Round 5 said the 2.36x does not follow from "96.5% headless vs 71.2% device" because those are by-count vs by-time. Confirmed: a device run under the corrected instrumentation, driven by a recorded take (13,864 frames of real gameplay), gives **by-COUNT 65.4%** (`fast=157670 slow=83250`) and **by-TIME 71.0%** (`fast=84079ms slow=34285ms`) — reproducing the register's 65.8% / 71.2% within run variance. **The like-for-like comparison is 96.5% headless vs 65.4% device = 1.48x, NOT 2.36x.** 🛑 The 8-beat question therefore does **not** follow from this ratio, and any He-Man verdict that stacked 2.36x on top of the bandwidth correction was double-counting. 🛑 **NEW, and it constrains the blend unit:** the slow path is **100% HARDLIGHT** (`[TBBSLOW] s4=525/24536K` — every slow blit is alpha=4). Tier-B cannot offload HARDLIGHT without first establishing what forces those blits to the CPU. ✅ **EXTENDED 2026-09-14 — ESTABLISHED LIBRARY-WIDE: the slow path IS THE gfxshadow.** A `[TBS]` counter tags the two shadow drawmethods (a separate END-of-struct field; the locked v3.10 line is untouched) and splits blits by kind x mode x path. Over the 450-PAK run (item 8): **gfxshadow is 93.2% of slow blits (1,390,973 of 1,492,450) and 66.8% of slow pixels, in 191 PAKs**; **every slow MULTIPLY blit is a gfxshadow** — `[TBS]` 1,216,051 equals `[TBBSLOW]` MULTIPLY 1,216,051 exactly; its other modes are HALF 102,436, HARDLIGHT 37,181, OVERLAY 33,506. **Sprite shadows are already fast**: 722,886 blits, all MULTIPLY, all on the fast path, 160 PAKs. The non-shadow slow remainder is **101,477 blits in 50 PAKs** (out-of-range 56,330, opaque 24,091, SCREEN 11,680, HALF 7,752, DODGE 1,624) but **33.2% of slow pixels** — real scale/rotate effects, concentrated. **Why gfxshadow cannot take the fast path:** its drawmethod sets `scaley` from `light.y`, `shiftx` from `light.x` and `fillcolor = shadowcolor` — each alone fails the `sprite.c` gate (`scaley == 256`, `!shiftx`, `fillcolor == TRANSPARENT_IDX`). 🛑 **So the blend unit needs a gfxshadow primitive — fill colour + vertical scale + horizontal shear, in the level's `shadowalpha` mode (MULTIPLY dominant) — not a general HARDLIGHT path.** 🛑 **Not yet confirmed on device:** the device He-Man HARDLIGHT above is INFERRED to be gfxshadow (`shadowalpha` 4); the headless He-Man scene reached a MULTIPLY gfxshadow (`k1m2S=403`). The mode follows the level's `shadowalpha`, so a device `[TBS]` run on the same take is what settles it | 14.4.5, 14.5 |
...
| # | item | where |
|---|---|---|
| 15b | `pitch % 16 == 0` -- the whole variant-selection determinism rests on it. 🛑 **RENUMBERED 2026-08-16: this row was also numbered 15**, colliding with the sprite-arena headroom item, so a reference to "item 15" was ambiguous — the same label-collision failure that lost round 5's E-3 (see item 14) | 9.6 |
| 16 | **E2**: the resolution scanner ignores `data/videopc.txt`. ✅ **CLOSED 2026-08-16 — MEASURED, zero impact.** The concern was REAL: `videopc.txt` wins outright when present (it replaces `filename` and `goto readfile`, so `video.txt` is never opened — `openbor.c:48548-48568`), and it applies to us because our `BUILD_MISTER` block sets `TARGET_PLATFORM = LINUX` (`apply_patches.py:217`). But the library says it changes nothing: **450 PAKs, 0 unreadable, exactly ONE ships `data/videopc.txt`** (Super Fightin' Spirit) and it is **byte-identical** to that PAK's `video.txt`. **8.2's R table, 9.6's variant counts and 14.4.5 are UNAFFECTED.** Tool: `tools/harness/pak_videopc_scan.py`, which tests byte-identity BEFORE parsing — `video.txt` has two forms (`video <w> <h>` and the mode-number `video <n>`) and a regex for one reports the other as a defect | 14.5, **pak_videopc_scan.py** |

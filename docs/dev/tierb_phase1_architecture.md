...
| 9 | **M11's DDR3 consumer enumeration.** 🛑 **PARTLY DONE 2026-08-15 — `docs/dev/tierb_ddr3_consumer_map.md`.** The SPLIT is now authoritative from the kernel cmdline (`mem=511M memmap=513M$511M`, device-tree size `0x40000000`): 1 GB total, Linux owns `0x0–0x1FEFFFFF`, **513 MB withheld at `0x1FF00000–0x3FFFFFFF`** — confirms §9.9.4's figure and supersedes its inference. Consumers enumerated with sources. **TWO FINDINGS:** (a) 🛑 **`ddr_svc`'s palette and the linux framebuffer are RUNTIME-programmed** (`pal_addr <= LFB_BASE − 4096`, `LFB_BASE` written by the HPS at `sys_top.v:443`) — **an arena cannot be placed by reading the RTL alone**; (b) the core window's **cart-data tail at `+0x80000` is unbounded** in every document. **STILL OPEN, and they are what block the memory map:** `ascal`'s framebuffer extent, and the core window's tail. **BUF2 (7a) stays folded here** — trivially placeable once a base is defensible, not before. 🛑 Do NOT mark this closed until both extents are measured | 6, 9.14, **ddr3_consumer_map** |

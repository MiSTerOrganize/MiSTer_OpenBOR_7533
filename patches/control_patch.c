 ...
 *
 * Attack3/Attack4 added 2026-07 so OpenBOR games that use all 6 action buttons
 * are fully playable. OpenBOR's standard layout puts Attack3/Attack4 on the
 * shoulder bumpers -- Xbox LB and Xbox RB. In the CONF_STR these are the jn
 * L/R slots (J1,Attack,Jump,Special,Attack2,Attack3,Attack4,Start /
 * jn,A,B,X,Y,L,R,Start). Adding the two buttons before Start shifts Start from
 * bit 8 to bit 10 (the RBF + this binary must ship together). The FPGA already
 * writes the full 32-bit joystick word to DDR3, so no RTL change beyond the
 * CONF_STR labels was needed.
 *

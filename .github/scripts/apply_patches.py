...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    # ---------------------------------------------------------------------
    # Piece 1 (2026-06-07): de-virtualize the per-pixel blend call.
    # putsprite_blend_ / _flip_ call blendfp(...) once PER PIXEL (an indirect
    # function-pointer call). The blend math itself is cheap -- 256x256 table
    # lookups, confirmed active at runtime via the [BLDTBL] diag -- so the
    # per-pixel INDIRECT CALL is the remaining overhead. blend_run32 /
    # blend_run32_flip pointer-match blendfp to the exact existing global blend
    # functions and inline the run loop (switch-once-per-run); an unrecognized
    # fp (e.g. tint / rgbchannel modes) falls through to the per-pixel indirect
    # call = today's exact behavior. Byte-identical by construction: each branch
    # calls the SAME global function, just inlined. No math duplicated.
    # ---------------------------------------------------------------------
    print("Patching pixelformat.c (Piece 1: blend_run32 de-virtualized run-blend)...")
    pixfmt_path = os.path.join(obor, 'source/gamelib/pixelformat.c')
    pf_p1 = read(pixfmt_path)
    blendrun_old = "blend32fp getblendfunction32(int alpha)\n{"
    blendrun_new = (
        "/* Piece 1 (MiSTer 2026-06-07): blend a contiguous run with the per-mode\n"
        " * function INLINED (switch-once-per-run), removing the per-pixel indirect\n"
        " * call in putsprite_blend_/_flip_. Byte-identical to blendfp(...) per pixel:\n"
        " * each branch calls the SAME global blend function; an unrecognized fp uses\n"
        " * the indirect call (today's exact behavior). */\n"
        "#define MISTER_BLEND_RUN_FWD(FN) do { int _i; for(_i = 0; _i < count; _i++) dest[_i] = FN(palette[idx[_i]], dest[_i]); } while(0)\n"
        "#define MISTER_BLEND_RUN_REV(FN) do { int _i; for(_i = 0; _i < count; _i++) { dest--; *dest = FN(palette[idx[_i]], *dest); } } while(0)\n"
        "void blend_run32(unsigned *dest, unsigned *palette, unsigned char *idx, int count, blend32fp fp)\n"
        "{\n"
        "    if(fp == blend_screen32)         MISTER_BLEND_RUN_FWD(blend_screen32);\n"
        "    else if(fp == blend_multiply32)  MISTER_BLEND_RUN_FWD(blend_multiply32);\n"
        "    else if(fp == blend_overlay32)   MISTER_BLEND_RUN_FWD(blend_overlay32);\n"
        "    else if(fp == blend_hardlight32) MISTER_BLEND_RUN_FWD(blend_hardlight32);\n"
        "    else if(fp == blend_dodge32)     MISTER_BLEND_RUN_FWD(blend_dodge32);\n"
        "    else if(fp == blend_half32)      MISTER_BLEND_RUN_FWD(blend_half32);\n"
        "    else { int _i; for(_i = 0; _i < count; _i++) dest[_i] = fp(palette[idx[_i]], dest[_i]); }\n"
        "}\n"
        "void blend_run32_flip(unsigned *dest, unsigned *palette, unsigned char *idx, int count, blend32fp fp)\n"
        "{\n"
        "    if(fp == blend_screen32)         MISTER_BLEND_RUN_REV(blend_screen32);\n"
        "    else if(fp == blend_multiply32)  MISTER_BLEND_RUN_REV(blend_multiply32);\n"
        "    else if(fp == blend_overlay32)   MISTER_BLEND_RUN_REV(blend_overlay32);\n"
        "    else if(fp == blend_hardlight32) MISTER_BLEND_RUN_REV(blend_hardlight32);\n"
        "    else if(fp == blend_dodge32)     MISTER_BLEND_RUN_REV(blend_dodge32);\n"
        "    else if(fp == blend_half32)      MISTER_BLEND_RUN_REV(blend_half32);\n"
        "    else { int _i; for(_i = 0; _i < count; _i++) { dest--; *dest = fp(palette[idx[_i]], *dest); } }\n"
        "}\n"
        "#undef MISTER_BLEND_RUN_FWD\n"
        "#undef MISTER_BLEND_RUN_REV\n"
        "\n"
        "blend32fp getblendfunction32(int alpha)\n"
        "{"
    )
    pf_p1 = strict_replace(pf_p1, blendrun_old, blendrun_new,
                           'Piece 1: insert blend_run32 / blend_run32_flip before getblendfunction32')
    write(pixfmt_path, pf_p1)
    print("  pixelformat.c: blend_run32 + blend_run32_flip inserted")

    print("Patching types.h (Piece 1: declare blend_run32 / blend_run32_flip)...")
    types_path = os.path.join(obor, 'source/gamelib/types.h')
    th_p1 = read(types_path)
    th_p1 = strict_replace(
        th_p1,
        "blend32fp getblendfunction32(int alpha);",
        "blend32fp getblendfunction32(int alpha);\n"
        "void blend_run32(unsigned *dest, unsigned *palette, unsigned char *idx, int count, blend32fp fp);\n"
        "void blend_run32_flip(unsigned *dest, unsigned *palette, unsigned char *idx, int count, blend32fp fp);",
        'Piece 1: declare blend_run32 / blend_run32_flip in types.h')
    write(types_path, th_p1)
    print("  types.h: blend_run32 declarations added")

    print("Patching spritex8p32.c (Piece 1: putsprite_blend_/_flip_ call blend_run32)...")
    sp32_p1 = read(sp32_path)
    sp32_p1 = strict_replace(
        sp32_p1,
        "            for(; count > 0; count--, lx++)\n"
        "            {\n"
        "                dest[lx] = blendfp(palette[*data++], dest[lx]);\n"
        "            }",
        "            /* Piece 1: de-virtualized run-blend (inlined per-mode). */\n"
        "            blend_run32(&dest[lx], palette, data, count, blendfp);\n"
        "            lx += count;\n"
        "            data += count;",
        'Piece 1: putsprite_blend_ -> blend_run32')
    sp32_p1 = strict_replace(
        sp32_p1,
        "            for(; count > 0; count--)\n"
        "            {\n"
        ...
        "                dest[lx] = blendfp(palette[*data++], dest[lx]);\n"
        "            }",
        "            /* Piece 1: de-virtualized reverse run-blend (inlined per-mode). */\n"
        "            blend_run32_flip(&dest[lx], palette, data, count, blendfp);\n"
        "            lx -= count;\n"
        "            data += count;",
        'Piece 1: putsprite_blend_flip_ -> blend_run32_flip')
    write(sp32_path, sp32_p1)
    print("  spritex8p32.c: putsprite_blend_/_flip_ now call blend_run32")

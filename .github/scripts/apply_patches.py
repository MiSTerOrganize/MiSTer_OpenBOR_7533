...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    s57_new = (
        ...
        "            else\n"
        "            {\n"
        "                /* Step 63: not in rolling state (e.g., ANI_DEATH from   */\n"
        "                /* player hit). Zero vel.x to halt motion -- otherwise   */\n"
        "                /* the engine main loop continues using last-set vel.x  */\n"
        "                /* and the barrel rolls forward during death animation. */\n"
        "                self->velocity.x = 0;\n"
        "            }\n"
        "        }\n"
        "        /* ── end Step 57 + 63 ────────────────────────────────────── */\n"

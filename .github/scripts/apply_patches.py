...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                /* FLAG_START is EXCLUDED. Opening the pause menu counted as a\n"
            "                 * take-over, so mrec_mode hit 0 on the same frame and the\n"
            "                 * Recording submenu then drew its IDLE branch -- 'Stop Playback'\n"
            "                 * could never be reached by pressing pause. The item exists to\n"
            "                 * stop a replay; let it. (PICO-8 fixed the same defect.) */\n"
            "                if(_mr_pos > 0 && (_mr_k & ~_mr_pl[_mr_p] & ~(u64)FLAG_START)) _mr_live = 1;\n"
            "                _mr_pl[_mr_p] = _mr_k;\n"
            "            }\n"

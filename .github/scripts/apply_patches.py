...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                  NativeVideoWriter_Notice(\"You took over - replay stopped. Progress is not being saved, reset the PAK to save\", 6); }\n"
            "                else\n"
            "                { printf(\"[REPLAY] finished (%ld frames) -- you have control\\n\", _mr_len);\n"
            "                  NativeVideoWriter_Notice(\"Replay finished - you have control. Progress is not being saved, reset the PAK to save\", 6); }\n"

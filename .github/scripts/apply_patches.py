...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                /* Saves are STILL isolated here and the handler wipes that scratch on\n"
            "                 * the next launch, so everything played from this point is discarded.\n"
            "                 * Lowering mrec_isolate mid-run is worse -- the run's saves are already\n"
            "                 * in the scratch, so switching would split the state across two roots.\n"
            "                 * Say so instead; the user can reset the PAK to save normally. */\n"
            "                NativeVideoWriter_Notice(\"Progress is not being saved - reset the PAK to save\", 6);\n"

...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    fps_print_new = (
        ...
        "                /* MiSTer 2026-06-09 TEMPORARY DIAG (REVERT AFTER MEASURED): */\n"
        "                /* dump living non-player entities to find which one hangs    */\n"
        "                /* the level wait (TMNT-RP construction barrel reap bug).      */\n"
        "                {\n"
        "                    int _bd;\n"
        "                    for (_bd = 0; _bd < ent_max; _bd++) {\n"
        "                        entity *_be = ent_list[_bd];\n"
        "                        if (_be && _be->exists && _be->modeldata.type != TYPE_PLAYER) {\n"
        "                            printf(\"[BDIAG] %s x=%d y=%d z=%d anim=%u velx=%d sub=%d type=%d hp=%d\\n\",\n"
        "                                   _be->modeldata.name ? _be->modeldata.name : \"(null)\",\n"
        "                                   (int)_be->position.x, (int)_be->position.y, (int)_be->position.z,\n"
        "                                   _be->animnum, (int)_be->velocity.x,\n"
        "                                   _be->modeldata.subtype, _be->modeldata.type,\n"
        "                                   (int)_be->energy_state.health_current);\n"
        "                        }\n"
        "                    }\n"
        "                }\n"

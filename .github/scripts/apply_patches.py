...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "            if(_mr_len >= _mr_cap)\n"
            "            {\n"
            "                /* TEMPORARY DIAG -- REVERT AFTER MEASURED. Initial cap 8192 -> 64\n"
            "                 * so a growth boundary comes round every second, and the marker\n"
            "                 * file forces the allocator refusal on demand. This exercises the\n"
            "                 * REAL handler/notice/recovery at a REAL growth boundary; it does\n"
            "                 * NOT prove the allocator itself, which would need ulimit -v and\n"
            "                 * would more likely fail somewhere in the 146 MB PAK load. */\n"
            "                long _mr_nc = _mr_cap ? _mr_cap*2 : 64;\n"
            "                /* Refuse INSTEAD of calling realloc, never after it: a successful\n"
            "                 * realloc may MOVE the block, so nulling _mr_nb afterwards would\n"
            "                 * leave _mr_buf dangling and the mode==0 sweep would free a stale\n"
            "                 * pointer. Skipping the call reproduces exactly what a real\n"
            "                 * failure leaves behind -- the old buffer, still valid. */\n"
            "                u64 *_mr_nb = (access(\"/tmp/openbor_oom_test\",F_OK)==0) ? NULL\n"
            "                            : (u64*)realloc(_mr_buf,(size_t)_mr_nc*9*sizeof(u64));\n"

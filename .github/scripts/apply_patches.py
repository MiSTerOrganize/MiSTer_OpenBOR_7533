...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    s44b_new = (
        "            /* MiSTer Step 44+52 TEMPORARY DIAG (REVERT AFTER MEASURED) -- */\n"
        "            /* Per-tick log of subtype-arrow entity state                  */\n"
        "            /* Step 52 adds: pos.x, vel.x, owner ptr, aironly_dir_seen     */\n"
        "            { static int _d_bg=0;\n"
        "              if ((self->modeldata.subtype & SUBTYPE_ARROW) && _d_bg < 80) { _d_bg++;\n"
    ...
    s48_new = (
        ...
        "                self->velocity.y += gravity * 100.0 / GAME_SPEED;\n"
        "            }\n"
        "            /* MiSTer Step 48 + 49 + 51 (2026-05-30): direct position.x      */\n"
        "            /* update for level-spawned aironly SUBTYPE_ARROW in ANI_IDLE.   */\n"
        "            /* Step 51 adds !self->owner gate so player-fired projectiles    */\n"
        "            /* (Bearz rocket launcher etc.) keep engine's arrow_move path   */\n"
        "            /* unchanged. Only level-spawned aironly arrows get this fix.   */\n"
        "            /*                                                                */\n"
        "            /* Step 49: engine's rest-on-ground branch at openbor.c:27784   */\n"
        "            /* zeroes vel.x because cart's anim idle has no per-frame move/  */\n"
        "            /* axis.y directives. Re-establish rolling speed if zeroed.    */\n"

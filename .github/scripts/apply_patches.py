...
"""
    ...
    video_c = strict_replace(
        ...
        '\tif(opengl) return video_gl_copy_screen(surface);\n'
        '\n'
        '\t/* Golden-master hash trace (debugging-methodology component 2).\n'
        '\t * Enabled by env OB_TEST=<tracefile>; default-off, fully inert\n'
        '\t * otherwise. Emits one FRAME:VIDEOCRC:AUDIOCRC line per presented\n'
        '\t * frame, hashing the engine\'s NATIVE-resolution framebuffer here\n'
        '\t * BEFORE any platform squish/scale (row-by-row, pitch padding\n'
        '\t * excluded). Runs in BOTH the MiSTer ship build and the headless\n'
        '\t * diff-harness build (this code sits before the ifdef split), so\n'
        '\t * hardware and x86 traces diff directly. AUDIOCRC is 00000000\n'
        '\t * until the fixed-timestep deterministic test mode lands\n'
        '\t * (methodology component 1) -- the mixer pull cadence is\n'
        '\t * asynchronous, so an audio hash here would be nondeterministic.\n'
        '\t * OB_TESTFRAMES=<N> exits clean after N traced frames. Trace FILE\n'
        '\t * is fully buffered -- no per-frame SD/disk I/O. */\n'
        '\t{\n'
        '\t\tstatic FILE *tt_f = NULL;\n'
        ...
        '\t\t\tif (tp && *tp) {\n'
        '\t\t\t\ttt_f = fopen(tp, "w");\n'
        '\t\t\t\tif (tt_f) setvbuf(tt_f, NULL, _IOFBF, 65536);\n'
        '\t\t\t}\n'
        '\t\t\ttt_limit = tl ? atol(tl) : 0;\n'
        '\t\t}\n'
        '\t\tif (tt_f) {\n'
        ...
        '\t\t\t\tfor (i = 0; i < 256; i++) {\n'
        '\t\t\t\t\tr = i;\n'
        '\t\t\t\t\tfor (j = 0; j < 8; j++) r = (r >> 1) ^ (0xEDB88320u & (0u - (r & 1u)));\n'
        '\t\t\t\t\ttt_tab[i] = r;\n'
        '\t\t\t\t}\n'
        '\t\t\t\ttt_have = 1;\n'
        '\t\t\t}\n'
        '\t\t\tfor (y = 0; y < surface->height; y++) {\n'
        '\t\t\t\tconst unsigned char *row = (const unsigned char *)surface->data + (size_t)y * surface->pitch;\n'
...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    ob = strict_replace(
        ...
        "                    * and the Play refusal, while the engine thread writes it. It happened\n"
        "                    * to work only because sleep()/fopen() at the top of that loop are\n"
        "                    * opaque calls the compiler cannot cache across -- a property of the\n"
        "                    * loop's shape, not of the declaration, and this build uses -flto. */\n"
        ...
        "                    * echo word at 0x0C once per frame (ST_WRITE_JOY0), so for up to\n"
        "                    * one frame after a press 0x0C still reports the OLD slot. The\n"
        "                    * poll would adopt that stale value and REVERT the press, then\n"
        "                    * correct itself a poll later -- reading as 'the slot sometimes\n"
        "                    * jumps back', and mis-slotting a Record made inside that window.\n"
        "                    * Skipping exactly one poll is sufficient and cannot stall: the\n"
        "                    * next poll is ~1 s later, long after the reader has rewritten\n"
        "                    * 0x0C. A simultaneous OSD move is simply picked up then.\n"
        "                    * Lives here beside mrec_slot because the publisher (pausemenu,\n"
        "                    * engine TU) and the consumer (the swap thread in sdl/sdlport.c)\n"
        "                    * are different translation units. */\n"
        ...
        + (
            ...
            "static int mrec_highest(const char *stem);   /* same reason; pausemenu() calls it */\n"
            "int mrec_arm_slot_play(int slot);   /* SINGLE source of the arm-a-slot policy. NOT static: sdl/sdlport.c's OSD 'Play Replay' path calls it too, and mrec_content_id/mrec_highest are static so it could not reproduce the policy itself even if we wanted a second copy. */\n"
            ...
            "int mrec_save_slot_marker(int slot){ (void)slot; return 1; }   /* pausemenu() is replaced in BOTH builds and calls this; headless has no session to carry, so it succeeds inertly. Returning 0 would make the headless menu take the refusal path. */\n"
            ...
            " * zeroes: pausemenu() sizes its surface from it, and allocscreen(0,0) would\n"
            " * take the !menuscreen fallback path -- harmless here, but a stub that\n"
            " * quietly changes which branch runs is a worse thing to leave lying around\n"
            " * than one that answers honestly. */\n"
    ...
    if not HEADLESS:
        ...
        ob = strict_replace(
            ...
            "/* Flat file copy, replacing a system(\"cp -r ...\") whose destination was\n"
            " * built by sprintf from the PAK-derived identity -- a crafted PAK filename\n"
            " * could close the quoting and run commands as root. Two directories of\n"
            " * plain files do not need a shell; PICO-8 does the same thing in C. */\n"
            "/* Defined in native_video_writer.c, which is linked into both builds. Draws\n"
            " * a short line at the top of the screen for a few seconds, POST-downscale.\n"
            ...
            " * passed every guard and replayed as a plausible divergence -- silent, and\n"
            " * indistinguishable from a game bug. */\n"
            "static unsigned int mrec_crc32(const void *buf, size_t len)\n"
            "{\n"
            "    static unsigned int tab[256]; static int built = 0;\n"
            "    const unsigned char *p = (const unsigned char*)buf;\n"
            "    unsigned int c; size_t i;\n"
            "    if(!built){ unsigned int j,k;\n"
            "        for(j=0;j<256;j++){ c=j; for(k=0;k<8;k++) c = (c&1)?(0xEDB88320u^(c>>1)):(c>>1); tab[j]=c; }\n"
            "        built = 1; }\n"
            "    c = 0xFFFFFFFFu;\n"
            ...
            "}\n"
            "\n"
            "/* openbor.c does not include this, and the take-payload loop uses opendir/\n"
            " * readdir to enumerate the arm snapshot. (mrec_copy_files needed it first;\n"
            " * that helper is gone with the .snapshots sidecar, the include is NOT --\n"
            " * removing it would break the payload loop.)\n"
            " * Its absence is invisible to the apply_patches dry-run and to the\n"
            " * integrity gate -- both only do text replacement -- so it surfaced only as\n"
            " * a CI compile error: \"unknown type name 'DIR'\". */\n"
            "#include <dirent.h>\n"
            "#include \"native_sha1.h\"   /* content identity (step 19); header-only, static */\n"
            "\n"
            "/* BOUNDED content hash for a .pak. Identity must be content, never a path:\n"
            " * a renamed v2 otherwise matches v1's name, plays, and desyncs silently --\n"
            " * which is indistinguishable from a game bug both to the player and to the\n"
            " * developer reading the report it produces.\n"
            " *\n"
            " * Bounded, NOT a full-file hash. PAKs here are 400 MB - 1.4 GB and full\n"
            " * hashing was measured at 20-70 s per arm, twice a session; that does not\n"
            " * make the feature slower, it makes it unused. Do not 'helpfully' widen this\n"
            " * later -- the trade was measured and rejected.\n"
            " *\n"
            " * A first/last-window hash provably does NOT work here: the packfile\n"
            " * directory lives at the TAIL and asset data in the middle, so a one-character\n"
            " * edit to a character.txt leaves size, directory, first 64 KB and last 64 KB\n"
            " * all identical. So hash instead:\n"
            " *   (a) the entire directory -- every entry's pns_len/filestart/filesize/name,\n"
            " *       which catches every repack, add, remove, reorder and resize; and\n"
            " *   (b) the full bytes of every data/** member ending .txt or .c (~0.5-2 MB),\n"
            " *       which catches the same-size logic edit that is the common mod.\n"
            " *\n"
            " * KNOWN GAP, accepted deliberately: this does not catch a same-size edit to a\n"
            " * BINARY asset. Hitboxes and timings come from the text, but sprite headers\n"
            " * feed offsetx/offsety and loadsprite centres, so 'cannot desync' is unproven.\n"
            " * The residual failure is caught downstream anyway -- an unnoticed asset\n"
            " * difference still ends in take-over with a visible notice. Revisit only if a\n"
            " * real desync is traced to it.\n"
            " *\n"
            " * Returns 0 on success. A failure is NEVER a zero hash: two unhashable PAKs\n"
            " * must not compare equal, and a partial walk must not produce a plausible\n"
            " * digest, so any malformed entry aborts the whole hash.\n"
            " *\n"
            " * static INLINE, not plain static: the Makefile builds with -Werror, so a\n"
            " * plain static that nothing calls yet is -Wunused-function and fails the ship\n"
            " * build. (build_headless.sh rewrites -Werror; build_mister_arm.sh does not.) */\n"
            "static inline int mrec_pak_hash(const char *path, unsigned char out[NSHA1_DIGEST_LEN])\n"
            "{\n"
            "    FILE *f; long fsize, pos, dend; unsigned int dir_off = 0; nsha1_ctx c; int ok = 1;\n"
            "    f = fopen(path, \"rb\");\n"
            "    if(!f) return -1;\n"
            "    if(fseek(f,0,SEEK_END)!=0){ fclose(f); return -1; }\n"
            "    fsize = ftell(f);\n"
            "    if(fsize < 8){ fclose(f); return -1; }\n"
            "    /* directory offset is the last 4 bytes; the directory runs from there to\n"
            "     * just before that field. */\n"
            "    if(fseek(f,fsize-4,SEEK_SET)!=0 || fread(&dir_off,4,1,f)!=1){ fclose(f); return -1; }\n"
            "    dend = fsize - 4;\n"
            "    if((long)dir_off < 0 || (long)dir_off > dend){ fclose(f); return -1; }\n"
            "\n"
            "    nsha1_init(&c);\n"
            "    if(nsha1_update_range(&c,f,(long)dir_off,(size_t)(dend-(long)dir_off))!=0){ fclose(f); return -1; }\n"
            "\n"
            "    pos = (long)dir_off;\n"
            "    while(pos + 12 <= dend)\n"
            "    {\n"
            "        unsigned int pns=0,start=0,size=0; int nl,i,L; char nm[4096];\n"
            "        if(fseek(f,pos,SEEK_SET)!=0){ ok=0; break; }\n"
            "        if(fread(&pns,4,1,f)!=1||fread(&start,4,1,f)!=1||fread(&size,4,1,f)!=1){ ok=0; break; }\n"
            "        if(pns<12||pns>4096){ ok=0; break; }   /* pns==0 would also spin forever */\n"
            "        nl = (int)pns - 12;\n"
            "        if(nl>0 && nl<(int)sizeof(nm))\n"
            "        {\n"
            "            if(fread(nm,1,(size_t)nl,f)!=(size_t)nl){ ok=0; break; }\n"
            "            nm[nl]=0;\n"
            "            /* normalise exactly as the engine does when it resolves a member */\n"
            "            for(i=0;i<nl;i++){ if(nm[i]=='\\\\') nm[i]='/'; if(nm[i]>='A'&&nm[i]<='Z') nm[i]=(char)(nm[i]+32); }\n"
            "            L = (int)strlen(nm);\n"
            "            if(L>5 && strncmp(nm,\"data/\",5)==0\n"
            "               && ((L>4 && strcmp(nm+L-4,\".txt\")==0) || (L>2 && strcmp(nm+L-2,\".c\")==0))\n"
            "               && size>0 && (long)start>=0 && (long)start+(long)size<=dend)\n"
            "            {\n"
            "                if(nsha1_update_range(&c,f,(long)start,(size_t)size)!=0){ ok=0; break; }\n"
            "            }\n"
            "        }\n"
            "        pos += (long)pns;\n"
            "    }\n"
            ...
            "}\n"
            "\n"
            "/* Validate a take BEFORE anything irreversible happens (design note S7).\n"
            " * Both 'play a replay' entry points -- the pause menu and the OSD .s1 pick --\n"
            " * used to write their markers and exit WITHOUT opening the file, so validation\n"
            " * happened in the NEXT process, after the PAK had already reloaded. A wrong\n"
            " * pick therefore destroyed the session and only then explained itself, which is\n"
            " * worst on the .s1 path because the OSD browser will hand us a take for any PAK\n"
            " * in the library.\n"
            " *\n"
            " * Reads the header and identity section only -- never the frame block or the\n"
            " * payload -- so it is cheap enough to run on every selection. Returns 1 if the\n"
            " * take would play; on 0, why[] holds a user-facing reason. */\n"
            "int mrec_probe_take(const char *path, char *why, int whysz)\n"
            "{\n"
            "    FILE *f; unsigned char mg[4], want[NSHA1_DIGEST_LEN], have[NSHA1_DIGEST_LEN];\n"
            "    unsigned int cont = 0, nfr = 0, pc = 0; unsigned short ic = 0, nl = 0, sl = 0; char nm[512];\n"
            "    f = fopen(path, \"rb\");\n"
            "    if(!f){ snprintf(why, whysz, \"That recording cannot be opened\"); return 0; }\n"
            "    if(fread(mg,1,4,f)!=4 || mg[0]!='M'||mg[1]!='R'||mg[2]!='E'||mg[3]!='C'\n"
            "       || fread(&cont,4,1,f)!=1)\n"
            ...
            "    /* Skip build + pak + seed, KEEP the frame count, skip crc, then read\n"
            "     * identity. The count is kept because the payload sits immediately past\n"
            "     * the frame block and the no-vouch check below has to find it; splitting\n"
            "     * the seek in two is cheaper than seeking back for it. */\n"
            "    if(fseek(f, MREC_OFF_FRAMES - MREC_OFF_BUILD, SEEK_CUR)!=0\n"
            "       || fread(&nfr,4,1,f)!=1\n"
            "       || fseek(f, MREC_HDR_BYTES - MREC_OFF_CRC, SEEK_CUR)!=0)\n"
            "    { fclose(f); snprintf(why, whysz, \"Recording is truncated - not playing\"); return 0; }\n"
            "    if(fread(&ic,2,1,f)!=1 || ic > 1)\n"
            "    { fclose(f); snprintf(why, whysz, \"Recording is damaged - not playing\"); return 0; }\n"
            "    if(ic == 0)\n"
            "    {   /* 🛑 NO IDENTITY ENTRY = NOTHING CHECKED THE PAK, so this take may\n"
            "         * play but may not RESTORE FILES. cont < 2 is already refused above,\n"
            "         * so ic == 0 here is a v2 take that DECLINED to vouch -- the writer\n"
            "         * emits 0 for an unhashable PAK or a name >= 512 chars. (The note\n"
            "         * that used to sit here called it a pre-v2 take, which the container\n"
            "         * check had made impossible.)\n"
            "         *\n"
            "         * Playing unverified is survivable: unverified input drives only the\n"
            "         * frame stream and the worst case is a visible desync. A payload is\n"
            "         * files written where the engine loads them, against a PAK nothing\n"
            "         * matched. The load path and mrec_extract_snap refuse the same case;\n"
            "         * refusing it HERE is what stops the user being reset to the title\n"
            "         * first, which is the entire reason this probe exists.\n"
            "         *\n"
            "         * SEQUENTIAL, not a computed offset. The identity count is followed by\n"
            "         * the recorder save STEM (u16 length + bytes) and only THEN the frame\n"
            "         * block -- a first version of this seeked to HDR+2+frames and landed\n"
            "         * inside the stem. Hand-computed offsets are what silently broke this\n"
            "         * format when the magic shrank 5 -> 4, which is why every other reader\n"
            "         * here parses forward. The payload count sits immediately after the\n"
            "         * frame block; a short or absent read means there is no payload to\n"
            "         * worry about, so it plays. */\n"
            "        if(fread(&sl,2,1,f)==1\n"
            "           && fseek(f, (long)sl + (long)nfr*MREC_FRAME_BYTES, SEEK_CUR)==0\n"
            "           && fread(&pc,4,1,f)==1 && pc > 0u)\n"
            "        { fclose(f); snprintf(why, whysz, \"Recording does not say which game - not playing\"); return 0; }\n"
            "        fclose(f); return 1;\n"
            "    }\n"
            "    if(fread(&nl,2,1,f)!=1 || nl==0 || nl >= (unsigned short)sizeof(nm)\n"
            "       || fread(nm,1,nl,f)!=nl || fread(want,1,NSHA1_DIGEST_LEN,f)!=NSHA1_DIGEST_LEN)\n"
            "    { fclose(f); snprintf(why, whysz, \"Recording is truncated - not playing\"); return 0; }\n"
            "    nm[nl] = 0;\n"
            "    fclose(f);\n"
            "    if(mrec_pak_hash(packfile, have) != 0 || memcmp(have, want, NSHA1_DIGEST_LEN) != 0)\n"
            "    { /* Same neutral wording and the same 24-char tail fit as the in-engine\n"
            "       * mismatch notice. `nm` is read straight out of a SHARED .inp, so it is\n"
            "       * attacker-influenced length as well as long-name-by-accident: unbounded,\n"
            "       * it pushes '- load it first' off the last line and the message loses the\n"
            "       * half that says what to DO. */\n"
            "      char sn[25]; size_t snl = strlen((const char *)nm);\n"
            "      if(snl <= 24) memcpy(sn, nm, snl + 1);\n"
            "      else { sn[0]='.'; sn[1]='.'; sn[2]='.'; memcpy(sn + 3, nm + (snl - 21), 21); sn[24]=0; }\n"
            "      snprintf(why, whysz, \"Needs %s - load it first\", sn); return 0; }\n"
            "    return 1;\n"
            "}\n"
            "\n"
            "/* A received take with an out-of-range index (FinalFight_37.inp) is reached\n"
            " * through the OSD 'Load Replay' slot, which plays an arbitrary file and\n"
            " * exists for exactly that. It is deliberately NOT reachable from the slot\n"
            " * picker: the picker addresses a BOUNDED library of MREC_SLOTS.\n"
            " *\n"
            " * An earlier version enumerated the directory to surface such a file here.\n"
            " * It could not work -- every caller clamps the result to MREC_SLOTS, so 37\n"
            " * fell through to slot 1, which is the exact failure enumeration was added\n"
            " * to prevent. Its claim that PICO-8 enumerates was also wrong: PICO-8\n"
            " * probes generated names (p8rec_highest). */\n"
            "/* Highest OCCUPIED slot. PROBES the 8 generated names -- it must never\n"
            " * PARSE them. The previous readdir version scanned filenames and pulled an\n"
            " * unbounded integer out of them, which was wrong twice over: a zero-padded\n"
            " * copy parsed to its bare value and the caller then rebuilt the unpadded\n"
            " * name, opening a file that does not exist; and a leftover from the old\n"
            " * unbounded library returned a value above MREC_SLOTS, which every caller\n"
            " * clamps -- landing on slot 1, the precise case the scan existed to handle.\n"
            " * Generating the names cannot drift from the loader and cannot return a\n"
            " * value outside 1..N. Matches PICO-8, which probes for the same reason. */\n"
            "static int mrec_highest(const char *stem)\n"
            "{\n"
            "    int i, best = 0; char p[MAX_BUFFER_LEN]; FILE *f;\n"
            "    for(i = 1; i <= MREC_SLOTS; i++)\n"
            "    {\n"
            "        snprintf(p, sizeof(p), \"/media/fat/replays/OpenBOR_7533/%s_%d.inp\", stem, i);\n"
            "        f = fopen(p, \"rb\");\n"
            "        if(f) { fclose(f); best = i; }\n"
            "    }\n"
            "    return best;\n"
            "}\n"
            "\n"
            "/* mrec_copy_files was removed with the .snapshots sidecar it existed to\n"
            " * write. Its only callers were there, and OpenBOR builds with -Werror,\n"
            " * where an unused static function fails the build. dirent.h stays\n"
            " * included -- the payload loop uses opendir as well. */\n"
            "\n"
            "/* SINGLE source of the arm-a-slot-for-playback policy.\n"
            " *\n"
            " * Two callers: the pause menu's Play Recording, and the OSD's\n"
            " * 'Play Replay' (sdl/sdlport.c's swap thread). They MUST refuse for the\n"
            " * same reasons with the same words -- a user who learns the feature in\n"
            " * one place must not meet different behaviour in the other -- so the\n"
            " * policy lives here once rather than being written twice. sdlport could\n"
            " * not reproduce it anyway: mrec_content_id and mrec_highest are static.\n"
            " *\n"
            " * Every refusal is EXPLAINED ON SCREEN and returns 0 without touching\n"
            " * anything. Order matters: an empty picked slot must say so even when\n"
            " * other slots hold takes, and a whole-library-empty PAK must say THAT\n"
            " * rather than blaming the slot.\n"
            " *\n"
            " * Returns 1 having written /tmp/openbor_playfile -- the caller then\n"
            " * writes recmode/reset markers and exits, because how it leaves differs\n"
            " * (pause menu exit(0) vs swap thread _exit(1)). */\n"
            "/* THE one writer of /tmp/openbor_recslot. Returns 1 on success.\n"
            " *\n"
            " * Every arm path exits the process and lets the daemon respawn us, so a\n"
            " * slot held only in memory dies with it -- this marker is how it travels.\n"
            " * If it does not land, the next process reads nothing, resolves to slot 1,\n"
            ...
            " * Mirrors PICO-8's p8rec_save_slot_marker, including the trailing newline. */\n"
            "int mrec_save_slot_marker(int slot)\n"
            "{\n"
            "    FILE *f;\n"
            "    if(slot < 1 || slot > MREC_SLOTS) slot = 1;\n"
            "    f = fopen(\"/tmp/openbor_recslot\", \"w\");\n"
            "    if(!f) { printf(\"[REC] cannot open the slot marker -- not arming\\n\"); return 0; }\n"
            "    if(fprintf(f, \"%d\\n\", slot) <= 0) { fclose(f); printf(\"[REC] slot marker write failed -- not arming\\n\"); return 0; }\n"
            "    if(fclose(f) != 0) { printf(\"[REC] slot marker close failed -- not arming\\n\"); return 0; }\n"
            "    return 1;\n"
            "}\n"
            "\n"
            "int mrec_arm_slot_play(int slot)\n"
            "{\n"
            "    char stem[MAX_BUFFER_LEN]; char path[MAX_BUFFER_LEN];\n"
            "    char why[128]; char msg[64]; FILE *f;\n"
            "    if(slot < 1 || slot > MREC_SLOTS) slot = 1;\n"
            "    mrec_content_id(stem, sizeof(stem));\n"
            "    if(mrec_highest(stem) <= 0)\n"
            "    {\n"
            "        printf(\"[REPLAY] no recording for this PAK -- not arming\\n\");\n"
            "        NativeVideoWriter_Notice(\"No recording for this game\", 4);\n"
            "        return 0;\n"
            "    }\n"
            "    snprintf(path, sizeof(path), \"/media/fat/replays/OpenBOR_7533/%s_%d.inp\", stem, slot);\n"
            "    f = fopen(path, \"rb\");\n"
            "    if(!f)\n"
            "    {\n"
            "        snprintf(msg, sizeof(msg), \"Slot %d is empty\", slot);\n"
            "        printf(\"[REPLAY] slot %d is empty -- not arming\\n\", slot);\n"
            "        NativeVideoWriter_Notice(msg, 4);\n"
            "        return 0;\n"
            "    }\n"
            "    fclose(f);\n"
            "    /* Validate BEFORE the reset. Arming a damaged take costs the user\n"
            "     * their session and explains itself only in the next process. */\n"
            "    if(!mrec_probe_take(path, why, (int)sizeof(why)))\n"
            "    {\n"
            "        printf(\"[REPLAY] not arming: %s\\n\", why);\n"
            "        NativeVideoWriter_Notice(why, 6);\n"
            "        return 0;\n"
            "    }\n"
            "    f = fopen(\"/tmp/openbor_playfile\", \"w\");\n"
            "    if(!f)\n"
            "    {\n"
            "        printf(\"[REPLAY] cannot write playfile -- not arming\\n\");\n"
            ...
            "        return 0;\n"
            "    }\n"
            "    /* Carry the slot BEFORE committing mrec_slot, so a failure here leaves\n"
            "     * nothing half-done. An unwritten marker resolves to slot 1 in the next\n"
            "     * process, so playback would open the WRONG take -- silently. */\n"
            "    if(!mrec_save_slot_marker(slot))\n"
            "    {\n"
            "        remove(\"/tmp/openbor_playfile\");   /* do not leave a stale one behind */\n"
            "        NativeVideoWriter_Notice(\"Could not start playback\", 5);\n"
            "        return 0;\n"
            "    }\n"
            "    mrec_slot = slot;   /* the slot the user just played is the slot both pickers should now show */\n"
            "    return 1;\n"
            "}\n"
            "\n"
            ...
            "     * attacker-controlled. Anything outside [A-Za-z0-9._-] becomes '_'. */\n"
            "    for(d = out; *d; d++)\n"
            "        if(!((*d >= 'A' && *d <= 'Z') || (*d >= 'a' && *d <= 'z')\n"
            "          || (*d >= '0' && *d <= '9') || *d == '.' || *d == '-' || *d == '_'))\n"
            "            *d = '_';\n"
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            "    {   /* MiSTer title-anchored raw-input recorder (menus + gameplay, interval-locked) */\n"
            "        static u64 *_mr_buf = NULL;\n"
            "        static long _mr_cap = 0, _mr_len = 0, _mr_pos = 0, _mr_capdone = 0;\n"
            ...
            "         * rewrites .hi, leaves the replay booting against a LOAD GAME menu of a\n"
            "         * different shape -- and the recorded navigation then selects a different\n"
            "         * slot. That is precisely the desync the snapshot exists to prevent.\n"
            ...
            "            _mr_armed = 1;\n"
            "            /* Drop a stale stop-marker before it can fire. /tmp is tmpfs, which\n"
            "             * survives a daemon respawn -- so if the process died between the menu\n"
            "             * creating this file and the recorder consuming it (an OSD PAK swap\n"
            "             * during a long load, a core switch, a kill), the marker outlives it.\n"
            "             * The next Record would then hit it on its very first frame and stop\n"
            ...
            "                    char _mr_b[MAX_BUFFER_LEN]; int _mr_mx = 0, _mr_armsl = mrec_slot;\n"
            ...
            "                    /* Highest OCCUPIED slot, used only as the fallback when no slot has been picked. mrec_highest PROBES the generated names (see its definition for why parsing them was wrong twice over); this used to enumerate the directory here, and _mr_i / _mr_miss were that loop's variables, kept alive afterwards by (void) casts alone. */\n"
            "                    _mr_mx = mrec_highest(_mr_nm);   /* bounded 1..MREC_SLOTS -- see mrec_highest */\n"
            ...
            "                    mrec_slot = _mr_armsl;   /* re-assert: adoption may have moved it while the fallback above was resolved */\n"
            "                    sprintf(_mr_path, \"%s_%d.inp\", _mr_b, _mr_armsl);   /* the slot the user ARMED, never one adopted mid-recovery */\n"
            "                }\n"
            "                if(_mr_m[0] == 'P')\n"
            "                {\n"
            "                    { FILE *_mr_ov = fopen(\"/tmp/openbor_playfile\", \"r\");\n"
            "                      if(_mr_ov)\n"
            "                      {   /* OSD 'Load Replay' (SC1) pick: play this exact .inp */\n"
            "                          char _mr_ovp[MAX_BUFFER_LEN]; _mr_ovp[0]=0;\n"
            "                          if(fgets(_mr_ovp, sizeof(_mr_ovp), _mr_ov))\n"
            "                          { char *_n=strchr(_mr_ovp,'\\n'); if(_n)*_n=0;\n"
            "                            char *_c=strchr(_mr_ovp,'\\r'); if(_c)*_c=0;\n"
            "                            if(_mr_ovp[0]) strcpy(_mr_path, _mr_ovp); }\n"
            "                          fclose(_mr_ov); remove(\"/tmp/openbor_playfile\"); } }\n"
            ...
            "                           && _mr_cv==2u   /* container version. v2 adds the identity section: content hash + recorder save stem. A mismatch short-circuits this whole chain and lands on the generic not-a-valid-recording message -- it is NOT separately reported here, and a note claiming it was stood at this line for a while. It costs nothing because mrec_probe_take runs first on BOTH entry points (the .s1 pick and mrec_arm_slot_play) and refuses cont != 2 with the older-core/newer-core wording BEFORE the content reset. This is the backstop behind that, so it is reached only if a future path loads without probing -- which is the thing to fix if this message is ever seen. */\n"
            ...
            "                           && _mr_len>0 && _mr_len<=2000000L)   /* <= : the writer lets _mr_len REACH 2000000, so < rejected a maximum-length recording the same build had just written. The size check moved BELOW the identity section, which is variable length, so the frame offset is measured rather than hand-summed -- the class of bug that made every take unreadable when the magic shrank 5 -> 4. */\n"
            "                        {\n"
            "                            /* ---- identity section (container v2) ------------------------\n"
            "                             * Sits between the fixed header and the frame block, so a reader\n"
            "                             * finds frames at whatever offset this ends. The seed stays at\n"
            "                             * MREC_OFF_SEED inside the FIXED header, so _handler.sh's dd is\n"
            "                             * unaffected by anything here.\n"
            "                             * A short read is TRUNCATION, not \"an older take\": the container\n"
            "                             * version above already rejected every older format. */\n"
            "                            unsigned char _mr_ih[NSHA1_DIGEST_LEN]; int _mr_have_id = 0;\n"
            "                            unsigned short _mr_ic = 0, _mr_nl = 0, _mr_sl = 0;\n"
            "                            char _mr_idn[512]; char _mr_stem[128];\n"
            "                            long _mr_fstart; int _mr_bad = 0;\n"
            "                            _mr_idn[0] = 0; _mr_stem[0] = 0;\n"
            "                            if(fread(&_mr_ic,2,1,_mr_pf)!=1 || _mr_ic > 1) _mr_bad = 1;\n"
            "                            else if(_mr_ic == 1)\n"
            "                            {\n"
            "                                if(fread(&_mr_nl,2,1,_mr_pf)!=1 || _mr_nl==0 || _mr_nl >= (unsigned short)sizeof(_mr_idn)) _mr_bad = 1;\n"
            "                                else if(fread(_mr_idn,1,_mr_nl,_mr_pf)!=_mr_nl) _mr_bad = 1;\n"
            "                                else if(fread(_mr_ih,1,NSHA1_DIGEST_LEN,_mr_pf)!=NSHA1_DIGEST_LEN) _mr_bad = 1;\n"
            "                                else { _mr_idn[_mr_nl] = 0; _mr_have_id = 1; }\n"
            "                            }\n"
            "                            /* Recorder's save stem. .sav/.hi/.sNN derive from getPakName, so a\n"
            "                             * recipient whose file is named differently has an engine looking\n"
            "                             * for names a shared payload does not contain. Stored here; the\n"
            "                             * REMAPPING belongs with the extractor (step 21), because nothing\n"
            "                             * reads the payload yet and a local take's stem already matches. */\n"
            "                            if(!_mr_bad)\n"
            "                            {\n"
            "                                if(fread(&_mr_sl,2,1,_mr_pf)!=1 || _mr_sl >= (unsigned short)sizeof(_mr_stem)) _mr_bad = 1;\n"
            "                                else if(_mr_sl && fread(_mr_stem,1,_mr_sl,_mr_pf)!=_mr_sl) _mr_bad = 1;\n"
            "                                else _mr_stem[_mr_sl] = 0;\n"
            "                            }\n"
            "                            _mr_fstart = ftell(_mr_pf);\n"
            "                            if(_mr_bad || _mr_fstart < 0\n"
            "                               || fseek(_mr_pf,0,SEEK_END)!=0\n"
            "                               || ftell(_mr_pf) < _mr_fstart + _mr_len*MREC_FRAME_BYTES\n"
            "                               || fseek(_mr_pf,_mr_fstart,SEEK_SET)!=0)\n"
            "                            {   /* the file must actually HOLD the frames it claims, or a ~290\n"
            "                                 * byte header saying len=2000000 allocates 144 MB before the\n"
            "                                 * read fails -- repeatable, on a 1 GB board shared with the FPGA */\n"
            "                                printf(\"[REPLAY] %s is truncated or has a bad identity section -- not playing\\n\", _mr_path);\n"
            "                                NativeVideoWriter_Notice(\"Recording is truncated - not playing\", 5);\n"
            "                                _mr_len = 0;\n"
            "                            }\n"
            "                            else\n"
            "                            {\n"
            "                            /* CONTENT guard: NEVER play a replay recorded against different\n"
            "                             * BYTES (guaranteed desync). Compares the bounded PAK hash, not\n"
            "                             * the name. The name was wrong in both directions: a renamed or\n"
            "                             * in-place-modded v2 matched v1 and desynced silently -- which is\n"
            "                             * indistinguishable from a game bug, to the player and to whoever\n"
            "                             * reads the report -- while the same PAK in a different subfolder\n"
            "                             * was refused outright. The stored name now only lets the refusal\n"
            "                             * say WHICH PAK is wanted.\n"
            "                             *\n"
            "                             * A pre-v2 take carries no entry and cannot be verified. It still\n"
            "                             * plays rather than bricking every take in circulation; the\n"
            "                             * engine-version warning is the honest signal for that case. */\n"
            "                            unsigned char _mr_cw[NSHA1_DIGEST_LEN]; int _mr_mism = 0;\n"
            "                            int _mr_nv = 0; unsigned int _mr_pc0 = 0;\n"
            "                            _mr_pk[255] = 0;\n"
            "                            if(_mr_have_id)\n"
            "                            {   /* packfile: engine global holding the loaded .pak path */\n"
            "                                if(mrec_pak_hash(packfile, _mr_cw) != 0) _mr_mism = 1;   /* unhashable is NOT equal */\n"
            "                                else if(memcmp(_mr_cw, _mr_ih, NSHA1_DIGEST_LEN) != 0) _mr_mism = 1;\n"
            "                            }\n"
            "                            else\n"
            "                            {   /* 🛑 NO IDENTITY ENTRY = NOTHING CHECKED THE PAK.\n"
            "                                 * ic > 1 is refused above and ic == 1 is verified, so ic == 0\n"
            "                                 * fell straight through here with _mr_have_id = 0 and the\n"
            "                                 * compare never ran. One take then worked against EVERY PAK,\n"
            "                                 * which is what turned the round-14 payload hole from a\n"
            "                                 * targeted attack into a universal one.\n"
            "                                 *\n"
            "                                 * Not refused outright: the writer legitimately emits 0 when\n"
            "                                 * the PAK is unhashable or its name is >= 512 chars, and such\n"
            "                                 * a take should still play -- unverified input drives only the\n"
            "                                 * frame stream, and the worst case is a VISIBLE desync.\n"
            "                                 * Carrying a PAYLOAD is different in kind: those are files\n"
            "                                 * written into the scratch for the engine to load, chosen by\n"
            "                                 * whoever made the take, against a PAK nothing matched.\n"
            "                                 *\n"
            "                                 * The count sits immediately after the frame block, whose\n"
            "                                 * extent was just proven to exist by the size check above. The\n"
            "                                 * position is restored either way -- the frame read below\n"
            "                                 * starts from _mr_fstart. */\n"
            "                                if(fseek(_mr_pf, _mr_fstart + _mr_len*MREC_FRAME_BYTES, SEEK_SET)==0\n"
            "                                   && fread(&_mr_pc0,4,1,_mr_pf)==1 && _mr_pc0 > 0u) _mr_nv = 1;\n"
            "                                if(fseek(_mr_pf, _mr_fstart, SEEK_SET)!=0) _mr_nv = 1;\n"
            "                            }\n"
            "                            if(_mr_nv)\n"
            "                            {   printf(\"[REPLAY] %s carries save data but does not identify the PAK it was recorded from -- not playing\\n\", _mr_path);\n"
            "                                NativeVideoWriter_Notice(\"Recording does not say which game - not playing\", 6);\n"
            "                                _mr_len=0; }\n"
            "                            else if(_mr_mism)\n"
            "                            { char _mr_nb[160]; char _mr_sn[25]; size_t _mr_nl;\n"
            "                              printf(\"[REPLAY] loaded PAK does not match this recording (recorded from '%s', identity '%s') -- not playing; load the matching PAK first\\n\", _mr_pk, _mr_idn);\n"
            "                              /* On screen too: this is the message a SHARED recording\n"
            "                               * produces most often, and the log is unreadable from a\n"
            "                               * couch. Name the PAK they need, not just \\\"refused\\\". */\n"
            "                              /* Bound the name. _mr_pk is char[256] and this notice is\n"
            "                               * 52x3; unbounded, a long PAK name pushes the instruction\n"
            "                               * off the last line and the message loses the half that\n"
            "                               * says what to DO. Keep the TAIL -- the filename identifies\n"
            "                               * it, the leading folders are inferable. 24 chars, matching\n"
            "                               * PICO-8 P8REC_NAME_FIT so both cores behave the same. */\n"
            "                              _mr_nl = strlen(_mr_pk);\n"
            "                              if(_mr_nl <= 24) memcpy(_mr_sn, _mr_pk, _mr_nl + 1);\n"
            "                              else { _mr_sn[0]='.'; _mr_sn[1]='.'; _mr_sn[2]='.';\n"
            "                                     memcpy(_mr_sn + 3, _mr_pk + (_mr_nl - 21), 21); _mr_sn[24]=0; }\n"
            "                              snprintf(_mr_nb, sizeof(_mr_nb), \"Needs %s - load it first\", _mr_sn);\n"
            "                              NativeVideoWriter_Notice(_mr_nb, 6);\n"
            "                              _mr_len=0; }\n"
            "                            else\n"
            "                            {\n"
            "                                _mr_buf = (u64*)malloc((size_t)_mr_len*9*sizeof(u64));\n"
            "                                { int _mr_crcbad = 0;   /* A FLAG, not a Notice(). The || arm below\n"
            "                                                         * used to fire \"damaged\" and then evaluate 0,\n"
            "                                                         * so control fell into the else, which fired\n"
            "                                                         * \"truncated\" over it in the same frame --\n"
            "                                                         * Notice() keeps one buffer and the last call\n"
            "                                                         * wins. Bit-rot therefore reported as a short\n"
            "                                                         * file. Choose the message once, in the else. */\n"
            "                                if(_mr_buf && fread(_mr_buf,9*sizeof(u64),(size_t)_mr_len,_mr_pf)==(size_t)_mr_len\n"
            "                                   && (mrec_crc32(_mr_buf,(size_t)_mr_len*72) == _mr_crc\n"
            "                                       || (printf(\"[REPLAY] %s failed its checksum -- damaged, not playing\\n\", _mr_path),\n"
            "                                           _mr_crcbad = 1, 0)))\n"
            "                                { _mr_pos=0; mrec_mode=2; srand32(_mr_seed);\n"
            "                                  /* REFUSE on a version mismatch -- do not play best-effort.\n"
            "                                   * (Policy changed 2026-08-05 by user decision; this core used\n"
            "                                   * to play anyway and say \"may not match\".)\n"
            "                                   *\n"
            "                                   * MREC_ENGINE_VER is bumped ONLY for a shipped game-LOGIC\n"
            "                                   * change that would desync old takes -- never for render,\n"
            "                                   * audio, UI or perf. So a mismatch is not a RISK of desync,\n"
            "                                   * it is a known one, and playing it hands the player a replay\n"
            "                                   * that drifts into nonsense with no way to tell that from a\n"
            "                                   * bug in the core. Say which direction it is, so the message\n"
            "                                   * names something they can DO -- matching PICO-8. */\n"
            "                                  if(_mr_ev != MREC_ENGINE_VER)\n"
            "                                  { _mr_bd[11]=0;\n"
            "                                    printf(\"[REPLAY] recorded on engine v%u (build %s); this build is v%u -- not playing, it would desync\\n\", _mr_ev, _mr_bd, (unsigned int)MREC_ENGINE_VER);\n"
            "                                    if(_mr_ev > MREC_ENGINE_VER)\n"
            ...
            "                                else\n"
            "                                {   /* Was SILENT. A truncated .inp -- the likeliest\n"
            "                                     * corruption, and what a full SD during Stop used to\n"
            "                                     * produce -- lands HERE, not on the bad-magic branch\n"
            "                                     * below, so Play Recording reset the PAK and then just\n"
            "                                     * played normally with nothing saying why. */\n"
            "                                  printf(\"[REPLAY] %s is truncated or out of memory (expected %ld frames) -- not playing\\n\", _mr_path, _mr_len);\n"
            "                                  NativeVideoWriter_Notice(_mr_crcbad\n"
            "                                      ? \"Recording is damaged - not playing\"\n"
            "                                      : \"Recording is truncated - not playing\", 5);\n"
            "                                  if(_mr_buf){free(_mr_buf);_mr_buf=NULL;} _mr_len=0; } }\n"
            "                            }\n"
            "                            }   /* closes the identity-section else */\n"
            "                        } else { printf(\"[REPLAY] %s is not a valid recording (wrong magic, truncated, or empty) -- not playing\\n\", _mr_path); NativeVideoWriter_Notice(\"That file is not a valid recording\", 5); _mr_len=0; }   /* was silent: Play Recording reloaded the PAK and then just played normally, with nothing anywhere saying why */\n"
            "                        fclose(_mr_pf);\n"
            "                    }\n"
            "                    else\n"
            "                    {   /* No else at all before this: a missing file produced NOTHING.\n"
            "                         * Reachable whenever pause-menu Play finds no takes -- the path\n"
            "                         * falls back to <id>_1.inp, the open fails, and the user's\n"
            "                         * session was destroyed by the reset for no reason and with no\n"
            "                         * explanation. */\n"
            "                        /* MISSING and UNREADABLE are different problems with different\n"
            "                         * answers, and this said \"no recording\" for both. A take that is\n"
            "                         * SITTING THERE and will not open -- bad permissions, a failing\n"
            "                         * card, a half-written file -- told the user their recording did\n"
            "                         * not exist, which sends them off to record it again instead of\n"
            "                         * looking at the card. PICO-8 has split these since it shipped. */\n"
            "                        { FILE *_mr_st = fopen(_mr_path, \"rb\"); int _mr_there = 0;\n"
            "                          if(_mr_st){ _mr_there = 1; fclose(_mr_st); }\n"
            "                          if(_mr_there)\n"
            "                          { printf(\"[REPLAY] %s exists but could not be opened\\n\", _mr_path);\n"
            "                            NativeVideoWriter_Notice(\"That recording cannot be opened\", 5); }\n"
            "                          else\n"
            "                          { /* \"No recording for this game\" is about the LIBRARY, not this\n"
            "                             * slot. It used to fire for a single missing file, so picking\n"
            "                             * slot 3 with takes in 1 and 2 said the PAK had none at all.\n"
            "                             * mrec_highest() answers the real question and _mr_nm is the\n"
            "                             * identity-derived stem already resolved above. */\n"
            "                            printf(\"[REPLAY] no recording found at %s\\n\", _mr_path);\n"
            "                            if(mrec_highest(_mr_nm) > 0)\n"
            "                            { char _mr_se[64];\n"
            "                              snprintf(_mr_se, sizeof(_mr_se), \"Slot %d is empty\", mrec_slot);\n"
            "                              NativeVideoWriter_Notice(_mr_se, 4); }\n"
            "                            else\n"
            "                              NativeVideoWriter_Notice(\"No recording for this game\", 4); } }\n"
            "                        _mr_len = 0;\n"
            "                    }\n"
            "                    /* A replay that never started must NOT leave the session isolated.\n"
            "                     * mrec_isolate is raised at openborMain entry and was never lowered,\n"
            "                     * so after ANY refusal above the player carried on in a normal\n"
            "                     * session whose saves resolved to the scratch -- which the handler\n"
            "                     * wipes on the next launch. An hour of progress, gone silently. */\n"
            "                    if(_mr_len == 0 && mrec_mode != 2)\n"
            "                    {\n"
            "                        mrec_isolate = 0;\n"
            "                        printf(\"[REPLAY] not playing -- saves restored to normal for this session\\n\");\n"
            "                    }\n"
            "                }\n"
            "                else\n"
            "                {\n"
            ...
            "            }\n"
            "        }\n"
            "        /* NOTE: there is deliberately NO drop-the-pause-frame block here.\n"
            "         * An earlier fix removed the frame whose press opened the pause menu,\n"
            "         * reasoning that the engine does not tick while paused so the pause\n"
            "         * could be elided from the recording. That is true of the PAUSED frames\n"
            "         * -- but NOT of the frame that opens the menu: pausemenu() is called at\n"
            "         * the END of update(), AFTER inputrefresh() and AFTER the\n"
            ...
            "             * that sets mode 0 from outside strands the allocation for the life\n"
            "             * of the process -- pause-menu Stop Playback does exactly that (its\n"
            "             * 'recorder frees next frame' comment was wrong: the free it meant is\n"
            "             * gated on mode==2, which is no longer true by then), and so does the\n"
            "             * realloc-failure path below. Up to ~140 MB on a 1 GB board shared\n"
            "             * with the FPGA. Ungated on _pause deliberately, like the drop above. */\n"
            "            free(_mr_buf); _mr_buf = NULL; _mr_cap = 0; _mr_len = 0; _mr_pos = 0; _mr_capdone = 0;\n"
            "        }\n"
            "        if(mrec_mode == 1)\n"
            ...
            "            if(_mr_s)\n"
            "            {\n"
            "                int _mr_ovw = 0;   /* did this slot already hold a take? Decided by the\n"
            "                                    * probe below, CONSUMED after the rename -- see there. */\n"
            "                fclose(_mr_s); remove(\"/tmp/openbor_recstop\");\n"
            "                {   /* bounded slot library: save to <pak>_<slot>.inp, slot 1..MREC_SLOTS */\n"
            "                    char _mr_b[MAX_BUFFER_LEN]; int _mr_mx = 0;\n"
            "                    strcpy(_mr_b, \"/media/fat/replays/OpenBOR_7533/\"); mrec_content_id(_mr_nm, MAX_BUFFER_LEN);   /* Identity-derived stem, NOT getPakName's basename: same-named PAKs in different Paks/ subfolders would otherwise share one library AND pass the match guard, and the wrong replay would PLAY. Also drops the old strstr(\".inp\") strip, which cut at the FIRST match and so mangled any PAK with .inp in its own name. */\n"
            "                    { size_t _mr_bl = strlen(_mr_b); snprintf(_mr_b + _mr_bl, sizeof(_mr_b) - _mr_bl, \"%s\", _mr_nm); }   /* was strcat, bounded only by upstream MAX_FILENAME_LEN=256 in a different file */\n"
            "                    /* Highest OCCUPIED slot, used only as the fallback when no slot has been picked. mrec_highest PROBES the generated names (see its definition for why parsing them was wrong twice over); this used to enumerate the directory here, and _mr_i / _mr_miss were that loop's variables, kept alive afterwards by (void) casts alone. */\n"
            "                    _mr_mx = mrec_highest(_mr_nm);   /* bounded 1..MREC_SLOTS -- see mrec_highest */\n"
            ...
            "                     * With no slot chosen this clamps to slot 1, matching\n"
            "                     * PICO-8 (p8rec_slot_now). That is a DECISION, not an\n"
            "                     * oversight: this used to scan for the first FREE slot so a\n"
            "                     * first-timer could not clobber anything, but it made the\n"
            "                     * menu open somewhere different depending on what had\n"
            "                     * already been recorded -- unpredictable exactly where a\n"
            "                     * user wants predictability. PICO-8 dropped first-free for\n"
            "                     * the same reason; this is the matching half. Overwriting\n"
            "                     * stays VISIBLE instead of prevented: the row reads used or\n"
            "                     * empty before you commit, and the notice names the slot\n"
            "                     * after. Do not restore first-free on either core.\n"
            "                     *\n"
            "                     * It was also already dead: the OSD poll adopts a 1..8 slot\n"
            "                     * within about a second of launch, so mrec_slot is in range\n"
            "                     * long before Record can run. */\n"
            ...
            "                      /* DETECT here, ANNOUNCE after the rename. This used to fire the\n"
            "                       * Notice() itself, before the .part was written, embedded,\n"
            "                       * closed and renamed -- asserting the outcome before any of it\n"
            "                       * ran. It read as correct only because the failure arm\n"
            "                       * overwrites the buffer in the same frame, which is an\n"
            "                       * invisible mitigation that breaks the moment a failure path\n"
            "                       * returns without a notice. PICO-8 announces after its rename. */\n"
            "                      if(_mr_ex){ fclose(_mr_ex); _mr_ovw = 1;\n"
            "                        printf(\"[REC] slot %d already held a take -- overwriting\\n\", mrec_slot); }\n"
            "                      else\n"
            "                        printf(\"[REC] saving to empty slot %d\\n\", mrec_slot); }\n"
            "                    }\n"
            "                }\n"
            "                if(_mr_len <= 0 || !_mr_buf)\n"
            "                {   /* NEVER create an empty .inp. It would take the highest index,\n"
            "                     * and pause-menu Play always opens the highest -- so one dud\n"
            "                     * permanently shadows every real take for this PAK, silently. */\n"
            "                    printf(\"[REC] nothing captured -- not writing a file\\n\");\n"
            "                    NativeVideoWriter_Notice(\"Nothing captured - no file. Saves off until reset.\", 4);\n"
            "                    mrec_mode = 0;\n"
            "                }\n"
            "                else\n"
            "                {   /* Write to <path>.part and rename in only once every byte is\n"
            "                     * committed. A short file would otherwise hold the highest index\n"
            "                     * and shadow the library exactly like the empty one. Every fwrite\n"
            "                     * and the fclose are checked -- the payload is buffered, so most\n"
            "                     * of a full-disk failure surfaces at close, not at write. */\n"
            "                    char _mr_tmp[MAX_BUFFER_LEN]; int _mr_ok = 0; FILE *_mr_wf;\n"
            "                    sprintf(_mr_tmp, \"%s.part\", _mr_path);\n"
            "                    _mr_wf = fopen(_mr_tmp, \"wb\");\n"
            "                    if(_mr_wf)\n"
            "                    { unsigned int _mr_ev = MREC_ENGINE_VER; char _mr_bd[12];\n"
            "                      /* STABLE magic + a separate container version. The version used\n"
            "                       * to live IN the magic, so a take from a NEWER build failed the\n"
            "                       * magic test and was reported as corrupt -- telling the user their\n"
            "                       * good file was damaged when their CORE was old. Receiving a newer\n"
            "                       * take is routine for a shared format. */\n"
            "                      { unsigned int _mr_cv = 2u;   /* v2: adds the identity section */\n"
            "                        _mr_ok = fwrite(\"MREC\",1,4,_mr_wf)==4;\n"
            "                        _mr_ok = _mr_ok && fwrite(&_mr_cv,sizeof(unsigned int),1,_mr_wf)==1; }\n"
            ...
            "                      /* FIXED width, not long. long is 4 bytes on armhf and 8 on a\n"
            "                       * 64-bit producer, so a desktop tool or headless harness would\n"
            "                       * emit 8 while the ARM reader consumed 4 -- getting a plausible\n"
            "                       * count and then reading the frame block 4 bytes out of phase,\n"
            "                       * with every validation passing. Every other field is fixed. */\n"
            "                      { unsigned int _mr_n32 = (unsigned int)_mr_len;\n"
            "                        unsigned int _mr_crc = mrec_crc32(_mr_buf, (size_t)_mr_len*72);\n"
            "                        _mr_ok = _mr_ok && fwrite(&_mr_n32,sizeof(unsigned int),1,_mr_wf)==1;\n"
            "                        _mr_ok = _mr_ok && fwrite(&_mr_crc,sizeof(unsigned int),1,_mr_wf)==1; }\n"
            "                      /* ---- identity section (v2) ---------------------------------\n"
            "                       * Content hash of the PAK, so identity is BYTES and not a name.\n"
            "                       * Written between the fixed header and the frame block; the reader\n"
            "                       * measures the frame offset rather than assuming one.\n"
            "                       * An unhashable PAK writes ZERO entries rather than a zero hash: the\n"
            "                       * take then does not vouch for its content, instead of vouching\n"
            ...
            "                          _mr_ok = _mr_ok && fwrite(&_mr_wl,2,1,_mr_wf)==1;\n"
            "                          _mr_ok = _mr_ok && fwrite(_mr_nm,1,_mr_wl,_mr_wf)==_mr_wl;\n"
            "                          _mr_ok = _mr_ok && fwrite(_mr_wh,1,NSHA1_DIGEST_LEN,_mr_wf)==NSHA1_DIGEST_LEN; }\n"
            "                        /* recorder save stem, for the extractor's remapping (step 21) */\n"
            "                        _mr_wstem[0]=0; getPakName(_mr_wstem, -1);\n"
            "                        _mr_wstem[sizeof(_mr_wstem)-1]=0;\n"
            "                        _mr_ws = (unsigned short)strlen(_mr_wstem);\n"
            "                        _mr_ok = _mr_ok && fwrite(&_mr_ws,2,1,_mr_wf)==1;\n"
            "                        if(_mr_ws) _mr_ok = _mr_ok && fwrite(_mr_wstem,1,_mr_ws,_mr_wf)==_mr_ws; }\n"
            "                      _mr_ok = _mr_ok && fwrite(_mr_buf,9*sizeof(u64),(size_t)_mr_len,_mr_wf)==(size_t)_mr_len;\n"
            "                      /* Embed the save snapshot so a take is ONE shareable file. A sidecar cannot\n"
            "                       * travel to another MiSTer, and its absence fails SILENTLY: the replay runs\n"
            "                       * against different save data, so the LOAD GAME menu has a different shape\n"
            "                       * and the recorded presses pick a different slot. The length is ALWAYS\n"
            "                       * written (0 when there is nothing), so the reader never has to guess. */\n"
            "                      /* Payload: length-prefixed records, NOT a tar. BusyBox tar -x has\n"
            "                       * varied across versions in how it treats ../ members and it follows\n"
            "                       * symlink members, which on a root filesystem is a persistence\n"
            "                       * primitive. Records are auditable in a way tar is not.\n"
            "                       *\n"
            "                       *   u32 count, then per file: u32 name_len, name, u32 data_len, data\n"
            "                       *\n"
            "                       * Same shape as PICO-8's so one reader parses both. The count is\n"
            "                       * ALWAYS written (0 when empty) so a reader never has to guess whether\n"
            "                       * a payload is present. Sourced from .armsnap -- the state the run\n"
            "                       * STARTED from, captured at arm -- not from the post-run scratch: a PAK\n"
            "                       * that autosaves on stage clear would otherwise pair the take with\n"
            "                       * progress the recorded navigation never saw, and the LOAD menu would\n"
            "                       * have a different shape on replay. */\n"
            "                      { unsigned int _mr_pc = 0; long _mr_cpos, _mr_epos; int _mr_di;\n"
            "                        _mr_cpos = ftell(_mr_wf);\n"
            "                        _mr_ok = _mr_ok && _mr_cpos >= 0 && fwrite(&_mr_pc,4,1,_mr_wf)==1;\n"
            "                        for(_mr_di = 0; _mr_ok && _mr_di < 2; _mr_di++)\n"
            "                        {\n"
            "                            char _mr_sd2[512]; DIR *_mr_d; struct dirent *_mr_e;\n"
            "                            snprintf(_mr_sd2,sizeof(_mr_sd2), \"%s/%s\", MREC_ARMSNAP, _mr_di ? \"savestates\" : \"saves\");\n"
            "                            _mr_d = opendir(_mr_sd2);\n"
            "                            if(!_mr_d) continue;   /* absent dir is legitimately empty */\n"
            "                            while(_mr_ok && (_mr_e = readdir(_mr_d)) != NULL)\n"
            "                            {\n"
            ...
            "                                if(!_mr_sf) continue;\n"
            "                                if(fseek(_mr_sf,0,SEEK_END)!=0){ fclose(_mr_sf); continue; }\n"
            "                                _mr_fl = ftell(_mr_sf);\n"
            ...
            "                                   || fwrite(_mr_e->d_name,1,_mr_nl2,_mr_wf)!=_mr_nl2\n"
            "                                   || fwrite(&_mr_dl,4,1,_mr_wf)!=1) { _mr_ok = 0; fclose(_mr_sf); break; }\n"
            "                                while(_mr_dl)\n"
            "                                {\n"
            "                                    char _mr_cb[8192];\n"
            "                                    size_t _mr_want = (_mr_dl < sizeof(_mr_cb)) ? (size_t)_mr_dl : sizeof(_mr_cb);\n"
            "                                    size_t _mr_got  = fread(_mr_cb,1,_mr_want,_mr_sf);\n"
            "                                    /* A short read here would desynchronise the whole record\n"
            "                                     * stream, so it fails the take rather than truncating one file. */\n"
            "                                    if(_mr_got != _mr_want || fwrite(_mr_cb,1,_mr_got,_mr_wf)!=_mr_got){ _mr_ok = 0; break; }\n"
            "                                    _mr_dl -= (unsigned int)_mr_got;\n"
            "                                }\n"
            "                                fclose(_mr_sf);\n"
            "                                if(_mr_ok) _mr_pc++;\n"
            "                            }\n"
            "                            closedir(_mr_d);\n"
            "                        }\n"
            "                        /* backfill the real count */\n"
            "                        if(_mr_ok)\n"
            "                        { _mr_epos = ftell(_mr_wf);\n"
            "                          _mr_ok = _mr_epos >= 0 && fseek(_mr_wf,_mr_cpos,SEEK_SET)==0\n"
            "                                   && fwrite(&_mr_pc,4,1,_mr_wf)==1\n"
            "                                   && fseek(_mr_wf,_mr_epos,SEEK_SET)==0; }\n"
            "                        if(_mr_ok) printf(\"[REC] embedded %u save file(s) in the take\\n\", _mr_pc); }\n"
            "                      if(fclose(_mr_wf) != 0) _mr_ok = 0;\n"
            "                    }\n"
            "                    if(_mr_ok && rename(_mr_tmp, _mr_path) == 0)\n"
            "                    {\n"
            "                        /* No .snapshots sidecar is written. The take already\n"
            "                         * carries these bytes: the embedded payload above and the\n"
            "                         * sidecar were BOTH copied from MREC_ARMSNAP, in this same\n"
            "                         * success block, so the directory duplicated the .inp it sat\n"
            "                         * beside. The .inp is self-sufficient and is the only form\n"
            "                         * that travels between machines.\n"
            "                         *\n"
            "                         * The handler does not read one either. A take predating\n"
            "                         * the payload is container v1, and the reader rejects it at\n"
            "                         * the _mr_cv==2u gate before any restore runs -- so the\n"
            "                         * fallback could not fire for any take that can exist. v2\n"
            "                         * and the payload shipped in the SAME binary, with no build\n"
            "                         * between them, so a v2 take without a payload never got\n"
            "                         * written either. */\n"
            "                        printf(\"[REC] wrote %s (%ld frames)\\n\", _mr_path, _mr_len);\n"
            "                        {   /* Now it is true. Folds in the length-limit case, whose own\n"
            "                             * notice fired a frame earlier and was overwritten by this\n"
            "                             * one ~10-16 ms later at 60-100 fps, so the player never\n"
            "                             * learned the take had been cut short.\n"
            "                             *\n"
            "                             * And it says the saves are still isolated. mrec_isolate is\n"
            "                             * raised at openborMain entry and lowered on exactly ONE path\n"
            "                             * (play-refusal), so everything written after Stop goes to\n"
            "                             * .scratch and is removed at the next launch. Lowering it\n"
            "                             * here would be worse -- the saves for this run are already in\n"
            "                             * the scratch, so writes would split across two directories --\n"
            "                             * and Reset does clear it, so the instruction is actionable. */\n"
            "                            char _mr_ow[96];\n"
            "                            if(_mr_capdone)\n"
            "                                snprintf(_mr_ow, sizeof(_mr_ow), \"Slot %d saved at the length limit. Saves off until reset.\", mrec_slot);\n"
            "                            else\n"
            "                                snprintf(_mr_ow, sizeof(_mr_ow), _mr_ovw\n"
            "                                    ? \"Replaced slot %d. Saves off until reset.\"\n"
            ...
            "                    }\n"
            "                    else\n"
            "                    {   /* KEEP the take in memory and stay in recording mode so the\n"
            "                         * user can free space and pick Stop Recording again, rather\n"
            "                         * than losing hours of capture with no message anywhere. */\n"
            "                        remove(_mr_tmp);\n"
            "                        printf(\"[REC] FAILED to write %s -- the recording is still in\"\n"
            "                               \" memory, free some space and pick Stop Recording again\\n\", _mr_path);\n"
            "                               NativeVideoWriter_Notice(\"Could not save the recording - free space and Stop again\", 7);\n"
            "                    }\n"
            "                }\n"
            "            }\n"
            "        }\n"
            "        if(mrec_mode == 1 && _mr_len >= 2000000L && !_mr_capdone)\n"
            ...
            "             * same function, so the flush runs on the NEXT frame -- and was then\n"
            "             * overwritten by the stop path's own notice. The stop path now folds\n"
            "             * the length-limit wording into its single, true announcement. */\n"
            "            _mr_capdone = 1;   /* ONCE. This block runs on EVERY frame\n"
            "                                * once the cap is reached, so without the\n"
            "                                * latch a write that FAILS -- the stop path\n"
            "                                * keeps the buffer and stays mode 1 on\n"
            "                                * purpose, so the user can free space and\n"
            "                                * Stop again -- is re-attempted as a full\n"
            "                                * file write every frame, with the notice\n"
            "                                * re-firing forever. Say it once; Stop\n"
            "                                * Recording is the retry, which is exactly\n"
            ...
            "             * pausemenu() is a modal loop that calls update(1,0) -> inputrefresh\n"
            "             * every iteration, so gating capture on !_pause meant the menu frames\n"
            "             * were never recorded -- and a replay that re-entered the menu then had\n"
            "             * NOTHING in the stream able to close it. That was the original hang,\n"
            "             * and both earlier attempts at it (dropping the opening frame, then\n"
            "             * refusing to open the menu) treated the symptom: the first deleted a\n"
            "             * frame that ticks in full and desynced the replay, the second skipped\n"
            "             * pausemenu()'s exit and left the engine's audio paused forever.\n"
            "             * Recording the menu frames makes the two runs symmetric -- the menu\n"
            "             * opens, the recorded navigation drives it, and the recorded Continue\n"
            "             * closes it, restoring audio on the way out exactly as it did live.\n"
            "             * This is the model PICO-8 uses, where the bug class does not exist. */\n"
            "            if(_mr_len >= _mr_cap)\n"
            "            {\n"
            "                long _mr_nc = _mr_cap ? _mr_cap*2 : 8192;\n"
            "                u64 *_mr_nb = (u64*)realloc(_mr_buf,(size_t)_mr_nc*9*sizeof(u64));\n"
            "                if(_mr_nb){ _mr_buf=_mr_nb; _mr_cap=_mr_nc; }\n"
            "                else\n"
            "                {   /* Out of memory. Say so -- silently dropping to idle meant the\n"
            "                     * player kept going for another hour believing they were still\n"
            ...
            "                { _mr_fr[1+_mr_p]=playercontrolpointers[_mr_p]->keyflags;\n"
            "                  _mr_fr[5+_mr_p]=playercontrolpointers[_mr_p]->newkeyflags; }\n"
            "                for(; _mr_p<4; _mr_p++){ _mr_fr[1+_mr_p]=0; _mr_fr[5+_mr_p]=0; }\n"
            "                _mr_len++;\n"
            "            }\n"
            "        }\n"
            "        if(mrec_mode == 2)\n"
            ...
            "                 * that does not trip take-over falls straight through and its\n"
            "                 * keyflags are OVERWRITTEN with the recorded value before the\n"
            "                 * caller's pause check ever sees it -- the menu simply never\n"
            "                 * opens. Taking over IS how a live pause reaches the engine:\n"
            "                 * take-over skips the injector, so keyflags keeps the live START\n"
            "                 * and the menu opens on that same press, under live control.\n"
            "                 * 'Stop Playback' is then unreachable because it is unnecessary\n"
            "                 * -- playback has already stopped, which is what was wanted. */\n"
            "                if(_mr_pos > 0 && (_mr_k & ~_mr_pl[_mr_p])) _mr_live = 1;\n"
            "                _mr_pl[_mr_p] = _mr_k;\n"
            "            }\n"
            "            if(_mr_live || _mr_pos >= _mr_len)\n"
            "            {   /* Two ENTIRELY different outcomes shared this branch with no\n"
            "                 * message at all -- not on screen, not even in the log. The\n"
            "                 * character simply stopped acting on its own, which looks the\n"
            "                 * same as a desync, a hang, or a bad recording. Take-over fires\n"
            "                 * on any newly-pressed button on any of four pads, so a sticky\n"
            "                 * d-pad or a second controller ends a replay at an arbitrary\n"
            "                 * frame; the player must be told which of the two happened. */\n"
            "                /* ONE notice, not two. Notice() writes a single static buffer, so a\n"
            "                 * second call in the same frame silently replaces the first: the\n"
            "                 * isolate warning below used to overwrite whichever of the two\n"
            "                 * outcomes had just been set, and the player never saw EITHER of\n"
            "                 * them -- defeating the whole point of this branch. Saves are STILL\n"
            "                 * isolated here and the handler wipes that scratch on the next\n"
            "                 * launch, so everything from this point is discarded; lowering\n"
            "                 * mrec_isolate mid-run is worse (the run's saves are already in the\n"
            "                 * scratch, so switching would split state across two roots). Say\n"
            "                 * both facts in one message. 52 cols x 3 lines is ample. */\n"
            "                if(_mr_live)\n"
            "                { printf(\"[REPLAY] you pressed a button at frame %ld -- replay stopped, you have control\\n\", _mr_pos);\n"
            "                  NativeVideoWriter_Notice(\"Took over - saves off until reset.\", 6); }\n"
            "                else\n"
            "                { printf(\"[REPLAY] finished (%ld frames) -- you have control\\n\", _mr_len);\n"
            "                  NativeVideoWriter_Notice(\"Replay over - saves off until reset.\", 6); }\n"
            "                if(_mr_buf){free(_mr_buf);_mr_buf=NULL;} _mr_cap=0; _mr_len=0; mrec_mode=0; }\n"
            "            else if(_mr_buf)\n"
            "            {\n"
            "                u64 *_mr_fr = _mr_buf + _mr_pos*9;\n"
            "                interval = (u32)_mr_fr[0];   /* force recorded step count -> _time replays identically (fixes desync) */\n"
            "                for(_mr_p=0; _mr_p<MAX_PLAYERS && _mr_p<4; _mr_p++)\n"
            "                { playercontrolpointers[_mr_p]->keyflags=_mr_fr[1+_mr_p];\n"

...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    loadingbar_new = (
        ...
        "                 * Slot 1 (loadingbg[1]) and bgPosi (per-level) keep\n"
        "                 * their cart-authored positions to avoid two\n"
        "                 * simultaneous bars on PAKs (Avengers UBF, PDC2) that\n"
        "                 * author BOTH loadingbg + loadingbg2 off-screen. */\n"
        "                if (s == &loadingbg[0] &&\n"
        "                    (pos_x < 0 || pos_y < 0 ||\n"

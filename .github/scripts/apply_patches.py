...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        ob = strict_replace(
            ...
            "    if(!f) { printf(\"[REPLAY] cannot write playfile -- not arming\\n\"); return 0; }\n"
            "    fputs(path, f); fclose(f);\n"
            "    mrec_slot = slot;   /* the slot the user just played is the slot both pickers should now show */\n"
            "    f = fopen(\"/tmp/openbor_recslot\", \"w\");\n"
            "    /* REFUSE if the slot cannot be carried across the reset. An\n"
            "     * unwritten marker resolves to slot 1 in the next process, so\n"
            "     * playback would open the WRONG take -- silently. Same rule and\n"
            "     * same trailing-newline format as PICO-8's p8rec_save_slot_marker. */\n"
            "    if(!f) { printf(\"[REPLAY] cannot write the slot marker -- not arming\\n\"); NativeVideoWriter_Notice(\"Could not start playback\", 5); return 0; }\n"
            "    fprintf(f, \"%d\\n\", slot); fclose(f);\n"
            "    return 1;\n"
            "}\n"
            "\n"

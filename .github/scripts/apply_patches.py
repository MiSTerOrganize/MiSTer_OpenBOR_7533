...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                                  { _mr_bd[11]=0; printf(\"[REPLAY] recorded on engine v%u (build %s); this build is v%u -- may desync, press any button to take control\\n\", _mr_ev, _mr_bd, (unsigned int)MREC_ENGINE_VER);\n"
            "                                    /* On screen too. This notice used to sit in the truncated-file\n"
            "                                     * branch below -- right call, wrong branch -- so a real engine\n"
            "                                     * version mismatch produced NOTHING on screen, only a log line\n"
            "                                     * nobody reads from a couch. */\n"

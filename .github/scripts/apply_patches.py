...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    s57_new = (
        "        }// end of if  - in-air checking\n"
        "        \n"
        "        /* MiSTer Step 57 + 59 (2026-05-31): rolling motion + friction. */\n"
        "        /* Fires every tick OUTSIDE the airborne block for aironly      */\n"
        "        /* SUBTYPE_ARROW + ANI_IDLE + !owner. Three behaviors combined: */\n"
        "        /*   (a) below cart spec speed  -> lock to target (handles      */\n"
        "        /*       engine bounce-halving + at-rest-branch zeroing).       */\n"
        "        /*   (b) above spec speed       -> friction decay 0.5% per tick */\n"
        "        /*       (Step 59: gives natural roll deceleration from the     */\n"
        "        /*        fall-energy bonus applied at landing in Step 46+59).  */\n"
        "        /*   (c) at spec speed          -> hold steady at target.       */\n"
        "        if (self->modeldata.subtype == SUBTYPE_ARROW\n"
        "            && self->modeldata.aironly_directive_seen\n"
        ...
        "        {\n"
        "            float _target_abs = self->modeldata.speed.x;\n"
        "            float _cur_abs = (self->velocity.x < 0) ? -self->velocity.x : self->velocity.x;\n"
        "            float _sign = (self->direction == DIRECTION_LEFT) ? -1.0f : 1.0f;\n"
        "            if (_cur_abs <= _target_abs)\n"
        "            {\n"
        "                /* Below or at floor -> lock to target. */\n"
        "                self->velocity.x = _sign * _target_abs;\n"
        "            }\n"
        "            else\n"
        "            {\n"
        "                /* Above floor (from fall-energy boost) -> friction decay. */\n"
        "                self->velocity.x *= 0.995f;\n"
        "                float _new_abs = (self->velocity.x < 0) ? -self->velocity.x : self->velocity.x;\n"
        "                if (_new_abs < _target_abs)\n"
        "                {\n"
        "                    self->velocity.x = _sign * _target_abs;\n"
        "                }\n"
        "            }\n"
    ...
    s46_new = (
        ...
        "                        checkdamageonlanding(self);\n"
        "\n"
        "                        /* MiSTer Step 46 + 59 (2026-05-31): aironly SUBTYPE_ARROW   */\n"
        "                        /* transitions ANI_FALL -> ANI_IDLE on landing + sets        */\n"
        "                        /* roll velocity. Step 59 adds 25 percent fall-energy bonus  */\n"
        "                        /* more natural rolling feel (vs the uniform locked vel.x    */\n"
        "                        /* in v3.10). Post-bounce vel.y is upward + halved-from-fall */\n"
        "                        /* magnitude, so fall_energy = post_bounce_vel.y * 2.       */\n"
        "                        if (self->modeldata.subtype == SUBTYPE_ARROW\n"
        "                            && self->modeldata.aironly_directive_seen\n"
        "                            && self->animnum != ANI_IDLE\n"
        ...
        "                            && self->modeldata.speed.x > 0)\n"
        "                        {\n"
        "                            float _fall_energy = (self->velocity.y > 0) ? (self->velocity.y * 2.0f) : 0.0f;\n"
        "                            float _bonus = _fall_energy * 0.25f;\n"
        "                            float _boosted = self->modeldata.speed.x + _bonus;\n"

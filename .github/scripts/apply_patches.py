...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                      /* Length ALWAYS written so the reader never has to guess whether\n"
            "                       * a payload is present -- but 0 for now, and the bytes withheld.\n"
            "                       *\n"
            "                       * Nothing reads this yet: playback still restores from the local\n"
            "                       * sidecar, so shipping the bytes was pure privacy and attack\n"
            "                       * surface for zero benefit. And it must NOT be a tar when it does\n"
            "                       * arrive -- BusyBox tar -x has varied across versions in how it\n"
            "                       * treats ../ members and it follows symlink members, which on a\n"
            "                       * root filesystem is a persistence primitive. The replacement is\n"
            "                       * PICO-8's length-prefixed record shape, which is auditable in a\n"
            "                       * way tar is not. See #INP_SHARING_DESIGN.md steps 4 and 8. */\n"
            "                      { unsigned int _mr_tl = 0;\n"
            "                        _mr_ok = _mr_ok && fwrite(&_mr_tl,sizeof(unsigned int),1,_mr_wf)==1; }\n"

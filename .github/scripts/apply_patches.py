...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    s57_new = (
        ...
        "        {\n"
        "            if (self->animnum == ANI_IDLE)\n"
        "            {\n"
        "                /* Rolling state -- lock vel.x to +/-speed.x.            */\n"
        ...
        "            else\n"
        "            {\n"
        "                /* Step 63 + 65: not in rolling state (e.g., ANI_DEATH   */\n"
        "                /* from player hit). Zero BOTH vel.x AND movex.         */\n"
        "                /* Engine main loop at line 29293 accumulates           */\n"
        "                /*   movex += vel.x * speedmul * factor                 */\n"
        "                /* BEFORE ent_post_update calls check_gravity. So when   */\n"
        "                /* the anim transitions to ANI_DEATH on tick N, movex  */\n"

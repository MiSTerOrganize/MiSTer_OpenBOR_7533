...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                      else\n"
            "                      { /* Confirm a fresh save too. Without this, Stop Recording on an\n"
            "                         * empty slot showed NOTHING at all -- the take was written and the\n"
            "                         * player had no way to know, while the overwrite case did report.\n"
            "                         * PICO-8 has said both since it shipped. */\n"

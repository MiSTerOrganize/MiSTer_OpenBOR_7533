...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    s41b_new = (
        "    case TYPE_PLAYER:\n"
        "        /* MiSTer Step 42 (2026-05-29): defensive force SUBJECT_TO_GRAVITY for      */\n"
        "        /* TYPE_PLAYER entities. Engine has ent_copy_uninit (openbor.c:23923) which */\n"
        "        /* unconditionally copies old model's move_config_flags via set_model_ex    */\n"
        "        /* (called on weapon swaps). After certain model transitions the player's   */\n"
        "        /* effective SUBJECT_TO_GRAVITY bit can be cleared, leading to respawn     */\n"
        "        /* going vertical-upward (no gravity to pull down). DIAG log proved this   */\n"
        "        /* manifests for Raph: model_gravity=1 at initial spawn, model_gravity=0   */\n"
        "        /* at respawn. Same defensive pattern as Step 31 v3 for TYPE_NONE.         */\n"
        "        e->modeldata.move_config_flags |= MOVE_CONFIG_SUBJECT_TO_GRAVITY;\n"
        "        /* MiSTer Step 41b TEMPORARY DIAG (REVERT AFTER MEASURED): log */\n"
        "        /* player entity creation state for respawn diagnosis. Now logs */\n"
        "        /* AFTER Step 42 force-set so we verify gravity is on.         */\n"

...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        ob = strict_replace(
            ...
            "}\n"
            "\n"
            "/* openbor.c does not include this, and both helpers below use opendir/\n"
            ...
            "}\n"
            "\n"
            "/* Highest take index for this content, by ENUMERATING Replays/ rather than\n"
            " * probing _1.._N. The probe stopped after 8 consecutive misses, so a take\n"
            " * received from someone else and dropped into an empty folder -- say\n"
            " * FinalFight_37.inp -- was invisible: Play opened _1.inp, reported it as not\n"
            " * a valid recording, and the next Record then wrote _1 and shadowed the\n"
            " * received file for good. That is a direct block on the whole point of\n"
            " * sharing. One directory scan is cheaper than 8-999 fopen pairs on exFAT\n"
            " * AND correct; PICO-8 has always enumerated. */\n"
            ...
            "        if(nl < sl + 6) continue;                 /* stem + '_' + digit + \".inp\" */\n"
            "        if(strncmp(n, stem, sl) != 0 || n[sl] != '_') continue;\n"
            "        if(strcmp(n + nl - 4, \".inp\") != 0) continue;\n"
            "        for(p = n + sl + 1; p < n + nl - 4; p++)\n"
            "        { if(*p < '0' || *p > '9') { v = -1; break; } v = v*10 + (*p - '0'); }\n"
            "        if(v > best) best = v;\n"
            "    }\n"
            "    closedir(d);\n"
            "    return best;\n"
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                              snprintf(_mr_nb, sizeof(_mr_nb), \"Recording is for %s - load that PAK\", _mr_pk);\n"

...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                      { char _mr_tc[MAX_BUFFER_LEN]; FILE *_mr_tf=NULL; unsigned int _mr_tl=0;\n"
            "                        sprintf(_mr_tc, \"cd '%s' && tar cf /tmp/mrec_snap.tar . 2>/dev/null\", MREC_ARMSNAP);\n"
            "                        if(system(_mr_tc)==0) _mr_tf = fopen(\"/tmp/mrec_snap.tar\",\"rb\");\n"
            "                        if(_mr_tf){ fseek(_mr_tf,0,SEEK_END); _mr_tl=(unsigned int)ftell(_mr_tf); fseek(_mr_tf,0,SEEK_SET); }\n"
            "                        _mr_ok = _mr_ok && fwrite(&_mr_tl,sizeof(unsigned int),1,_mr_wf)==1;\n"
            "                        if(_mr_tf){ char _mr_cb[8192]; size_t _mr_rd;\n"
            "                          while(_mr_ok && (_mr_rd=fread(_mr_cb,1,sizeof(_mr_cb),_mr_tf))>0)\n"
            "                            _mr_ok = _mr_ok && fwrite(_mr_cb,1,_mr_rd,_mr_wf)==_mr_rd;\n"
            "                          fclose(_mr_tf); remove(\"/tmp/mrec_snap.tar\"); }\n"
            "                        printf(\"[REC] embedded %u bytes of save data\\n\", _mr_tl); }\n"
            ...
            "                            sprintf(_mr_sc, \"mkdir -p '/media/fat/saves/OpenBOR_7533/.replays' && cp -r '/media/fat/saves/OpenBOR_7533/.replays/.armsnap' '/media/fat/saves/OpenBOR_7533/.replays/%.*s' 2>/dev/null\",\n"

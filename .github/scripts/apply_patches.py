...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "             * playback it re-enters the modal menu, where the recorder injects\n"
            "             * NOTHING -- inputrefresh IS still called in there, by the menu's own\n"
            "             * update(1,0), but capture and inject are both gated on !_pause and\n"
            "             * _pause is 2 in the menu -- so the replay hangs on the pause screen.\n"
            "             * Deterministic to elide: the engine does not tick while paused, so\n"
            "             * the replayed game timeline is unchanged.\n"
            "             * THIS BLOCK IS DELIBERATELY NOT GATED ON !_pause: it must run on the\n"
            "             * first inputrefresh INSIDE the menu. Do not tidy it under the gate --\n"
            "             * that strands mrec_drop_last and breaks the drop entirely.\n"
            "             * MUST run BEFORE the stop-signal flush below: pause -> Stop Recording is\n"
            "             * the usual way to end a recording, and flushing first would write that\n"
            "             * trailing press into the file (and clear mrec_mode, so the drop would\n"
            "             * then no-op). */\n"
            "            mrec_drop_last = 0;\n"

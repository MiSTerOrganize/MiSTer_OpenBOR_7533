...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                         *\n"
            "                         * The HANDLER still READS .snapshots as a fallback, for takes\n"
            "                         * recorded before the payload existed -- those have no embedded\n"
            "                         * section, so they would otherwise replay against the current\n"
            "                         * saves and desync with no message. Nothing writes new ones. */\n"

...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    s57_new = (
        ...
        "        {\n"
        "            /* Step 66 (2026-06-01): if entity is dying/dead, ALWAYS    */\n"
        "            /* halt regardless of animnum. Cart's anim death is a       */\n"
        "            /* single frame and engine's nodieblink=0 die-blink cycles  */\n"
        "            /* animnum back to ANI_IDLE during blink frames -- without  */\n"
        "            /* this gate, Step 57 sees ANI_IDLE and re-applies          */\n"
        "            /* vel.x = +/-speed.x mid-blink, causing barrel to roll     */\n"
        "            /* forward during death animation.                          */\n"
        ...
        "                self->movex = 0;\n"
        "            }\n"
        "            else if (self->animnum == ANI_IDLE)\n"
        "            {\n"
        "                /* Live rolling state -- lock vel.x to +/-speed.x.      */\n"
        ...
        "            else\n"
        "            {\n"
        "                /* Step 63 + 65: live but non-IDLE (e.g., ANI_FALL      */\n"
        "                /* recoil from hit). Zero BOTH vel.x AND movex.        */\n"
        "                /* Engine main loop at line 29293 accumulates           */\n"
        "                /*   movex += vel.x * speedmul * factor                 */\n"
        "                /* BEFORE ent_post_update calls check_gravity. So when   */\n"
        "                /* the anim transitions to ANI_FALL on tick N, movex   */\n"
        "                /* has already been populated with old vel.x. Step 63   */\n"
        "                /* alone (zeroing vel.x) doesn't stop the current tick's*/\n"
        "                /* motion -- trymove(movex) still uses old movex.       */\n"
        "                /* Step 65: also zero movex so check_move's trymove(0) */\n"
        "                /* halts the entity immediately.                        */\n"
        "                self->velocity.x = 0;\n"
        "                self->movex = 0;\n"
        "            }\n"
        "        }\n"
        "        /* ── end Step 57 + 63 + 65 + 66 ───────────────────────────── */\n"
        "        \n"
        "\t\tif(self->toss_time <= _time)\n"
    )

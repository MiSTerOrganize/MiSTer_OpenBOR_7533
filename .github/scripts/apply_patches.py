...
#define COPY_ROOT_PATH(buf, name) strcpy(buf, "./"); strcat(buf, name); strcat(buf, "/");
    ...
    if not HEADLESS:
        ...
        ob = strict_replace(
            ...
            "}\n"
            "\n"
            "/* openbor.c does not include this, and mrec_copy_files below uses opendir/\n"
            " * readdir. Its absence is invisible to the apply_patches dry-run and to the\n"
            ...
            "}\n"
            "\n"
            "static void mrec_copy_files(const char *sdir, const char *ddir)\n"
            "{\n"
            "    DIR *d; struct dirent *e;\n"
            "    char sp[MAX_BUFFER_LEN], dp[MAX_BUFFER_LEN], buf[8192];\n"
            "    size_t rd; FILE *fs, *fd;\n"
            "    d = opendir(sdir); if(!d) return;\n"
            "    while((e = readdir(d)) != NULL)\n"
            "    {\n"
            "        if(strcmp(e->d_name, \".\") == 0 || strcmp(e->d_name, \"..\") == 0) continue;\n"
            "        snprintf(sp, sizeof(sp), \"%s/%s\", sdir, e->d_name);\n"
            "        snprintf(dp, sizeof(dp), \"%s/%s\", ddir, e->d_name);\n"
            "        fs = fopen(sp, \"rb\"); if(!fs) continue;\n"
            "        fd = fopen(dp, \"wb\"); if(!fd) { fclose(fs); continue; }\n"
            "        while((rd = fread(buf, 1, sizeof(buf), fs)) > 0)\n"
            "            if(fwrite(buf, 1, rd, fd) != rd) break;\n"
            "        fclose(fd); fclose(fs);\n"
            "    }\n"
            "    closedir(d);\n"
            "}\n"
        ...
        rec_arm_new = rec_arm_anchor + "\n" + (
            ...
            "                    {\n"
            "                        {   /* Pair the take with the save state it STARTED from, so a\n"
            "                             * replay boots against the same data. Without it the LOAD\n"
            "                             * GAME menu would have a different shape later and the\n"
            "                             * recorded navigation would pick a different slot.\n"
            ...
            "                            /* Copied in C, NOT via system(). The destination is derived\n"
            "                             * from the PAK filename, and building a shell command from\n"
            "                             * that let a crafted name close the quoting and execute as\n"
            "                             * ROOT. The identity is whitelisted now, but a shell is the\n"
            "                             * wrong tool for copying two directories of files either way\n"
            "                             * -- this is what PICO-8's p8_copy_dir already does. */\n"
            "                            char _mr_sd[MAX_BUFFER_LEN], _mr_dd[MAX_BUFFER_LEN], _mr_ad[MAX_BUFFER_LEN];\n"
            "                            const char *_mr_bn = strrchr(_mr_path, '/');\n"
            "                            _mr_bn = _mr_bn ? _mr_bn + 1 : _mr_path;\n"
            "                            /* Keyed on the take's SEED, not just its name. Take names are\n"
            "                             * <content-id>_<N>.inp and that is identical on every machine\n"
            "                             * for the same PAK -- so a take copied in from someone else\n"
            "                             * whose name collided with one of yours used to restore YOUR\n"
            ...
            "                                  sprintf(_mr_sh + _mr_k*2, \"%02x\", _mr_sp[_mr_k]);\n"
            "                              snprintf(_mr_sd, sizeof(_mr_sd), \"%s/%.*s.%s\", MREC_REPSTATE,\n"
            "                                       (int)(strlen(_mr_bn) - 4), _mr_bn, _mr_sh); }\n"
            "                            mkdir(MREC_REPSTATE, 0777); mkdir(_mr_sd, 0777);\n"
            "                            snprintf(_mr_dd, sizeof(_mr_dd), \"%s/saves\", _mr_sd); mkdir(_mr_dd, 0777);\n"
            "                            snprintf(_mr_ad, sizeof(_mr_ad), \"%s/saves\", MREC_ARMSNAP);\n"
            "                            mrec_copy_files(_mr_ad, _mr_dd);\n"
            "                            snprintf(_mr_dd, sizeof(_mr_dd), \"%s/savestates\", _mr_sd); mkdir(_mr_dd, 0777);\n"
            "                            snprintf(_mr_ad, sizeof(_mr_ad), \"%s/savestates\", MREC_ARMSNAP);\n"
            "                            mrec_copy_files(_mr_ad, _mr_dd);\n"
            "                        }\n"

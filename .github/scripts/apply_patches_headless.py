...
def main():
    ...
    repl = ("\t{\n"
            ...
            "\t\t\t{   /* TEMPORARY DIAG [TBB] Tier-B Phase-1b blend histogram dump.\n"
            "\t\t\t     * Counters live in openbor.c (ship apply_patches.py, not\n"
            "\t\t\t     * HEADLESS-gated); its own report is gated on a wall clock the\n"
            "\t\t\t     * headless build never advances, so dump once here. */\n"
            "\t\t\t\textern unsigned int _mister_tbb_calls[8];\n"
            "\t\t\t\textern unsigned int _mister_tbb_px[8];\n"
            "\t\t\t\textern unsigned int _mister_tbb_tint, _mister_tbb_chan, _mister_tbb_remap;\n"
            "\t\t\t\textern unsigned int _mister_tb0_fast, _mister_tb0_slow;\n"
            "\t\t\t\textern unsigned int _mister_tb0_sprite_bytes, _mister_tb0_sprite_count;\n"
            "\t\t\t\tfprintf(stderr, \"[TBB] fast=%u slow=%u\", _mister_tb0_fast, _mister_tb0_slow);\n"
            "\t\t\t\t{ int _i; for (_i = 0; _i < 8; _i++) fprintf(stderr, \" m%d=%u\", _i, _mister_tbb_calls[_i]); }\n"
            "\t\t\t\t{ int _i; for (_i = 0; _i < 8; _i++) fprintf(stderr, \" p%d=%u\", _i, _mister_tbb_px[_i]); }\n"
            "\t\t\t\tfprintf(stderr, \" tint=%u chan=%u remap=%u frames=%ld\",\n"
            "\t\t\t\t        _mister_tbb_tint, _mister_tbb_chan, _mister_tbb_remap, _hl_n);\n"
            "\t\t\t\tfprintf(stderr, \" spritekb=%u sprites=%u vw=%d vh=%d\\n\",\n"
            "\t\t\t\t        _mister_tb0_sprite_bytes / 1024u, _mister_tb0_sprite_count,\n"
            "\t\t\t\t        stored_videomodes.hRes, stored_videomodes.vRes);\n"
            "\t\t\t}\n"
    ...
    # NOTE: the .inp recorder double-free fix now lives in the SHIP apply_patches.py
    # (step 2b), which the headless build also runs (before this patcher), so it
    # covers both the on-device Recording menu and the headless AI bot. It was
    # moved out of here to avoid a double-apply conflict.
